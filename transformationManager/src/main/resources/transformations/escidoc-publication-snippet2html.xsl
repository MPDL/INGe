...
<!--  Transformations from eSciDoc Citation Style Snippet format   into to the plain HTML snippets   Author: Vlad Makarenko (initial creation) $Author: $ (last changed)  $Revision: $ $LastChangedDate: $  -->
			...
			<xsl:element name="p">
				...
				<xsl:if test="$html_linked">
					...
					<xsl:for-each select="$comp/escidocComponents:content[@storage!='internal-managed']">
						<xsl:element name="a">
							<xsl:attribute name="href" select="@xlink:href"/>
							<xsl:attribute name="class" select="'Locator'"/>
							<xsl:element name="span">
								<xsl:value-of select="concat('[Locator ', position(), ']')"/>
							</xsl:element>
						</xsl:element>
					</xsl:for-each>
				</xsl:if>
