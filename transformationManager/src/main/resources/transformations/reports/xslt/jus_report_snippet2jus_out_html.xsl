        ...
        <xsl:element name="html">
            <xsl:element name="head">
                <xsl:element name="meta">
                    <xsl:attribute name="http-equiv" select="'Co ntent-Type'"></xsl:attribute>
                ...
                <xsl:element name="div">
                    ...
                    <xsl:element name="div">
                        ...
                        <xsl:for-each select="$coneResult/cone/rdf:RDF/rdf:Description">
                            ...
                            <xsl:variable name="currentAuthorId" select="@rdf:about"></xsl:variable>
                    ...
                    <xsl:element name="div">
                        ...
                        <xsl:variable name="firstEditorListCount" select="count($item-list/escidocItem:item/escidocMetadataRecords:md-records/escidocMetadataRecords:md-record[publication:publication/eterms:creator/@role = $editorRole and (publication:publication/@type = $genreCollectedEdition or publication:publication/@type = $genreMonograph or publication:publication/@type = $genreCommentary or publication:publication/@type = $genreHandbook or publication:publication/@type = $genreProceedings)])"></xsl:variable>
                        ...
                        <xsl:element name="div">
                            ...
                            <xsl:choose>
                                <xsl:when test="$firstEditorListCount &gt; 0">
                                    <xsl:text>SAMMEL- UND TAGUNGSBÄNDE/HERAUSGEBER- UND VERFASSUNGSWERKE</xsl:text>
                                    <xsl:text disable-output-escaping="yes">&lt;br&gt;</xsl:text>
                                </xsl:when>
                            </xsl:choose>
                            <xsl:for-each select="$coneResult/cone/rdf:RDF/rdf:Description">
                                <xsl:sort select="foaf:family_name"></xsl:sort>
                                <xsl:sort select="foaf:givenname"></xsl:sort>
                                <xsl:variable name="currentFirstEditorId" select="@rdf:about"></xsl:variable>
                                    ...
                                    <xsl:when test="count($item-list/escidocItem:item/escidocMetadataRecords:md-records/escidocMetadataRecords:md-record[publication:publication/eterms:creator/person:person/dc:identifier = $currentFirstEditorId and publication:publication/eterms:creator/@role = $editorRole]) &gt; 0">
                                        <xsl:element name="p">
                                            ...
                                            <xsl:apply-templates mode="sortedAuthorList" select="$currentFirstEditorList">
                                                ...
                                                <xsl:with-param name="authorCitationName" select="$currentFirstEditorCitationStyleName"
                                                ></xsl:with-param>
                                            </xsl:apply-templates>
                                        </xsl:element>
                                    </xsl:when>
                        ...
                        <xsl:element name="div">
                            ...
                            <xsl:for-each select="$coneResult/cone/rdf:RDF/rdf:Description">
                                <xsl:sort select="foaf:family_name"></xsl:sort>
                                <xsl:sort select="foaf:givenname"></xsl:sort>
                                <xsl:variable name="currentSecondEditorId" select="@rdf:about"></xsl:variable>
                                    ...
                                    <xsl:when test="count($item-list/escidocItem:item/escidocMetadataRecords:md-records/escidocMetadataRecords:md-record[publication:publication/eterms:creator/person:person/dc:identifier = $currentSecondEditorId and publication:publication/eterms:creator/@role = $editorRole and (publication:publication/@type = $genreJournal or publication:publication/@type = $genreSeries)]) &gt; 0">
                                        <xsl:element name="p">
                                            ...
                                            <xsl:apply-templates mode="sortedAuthorList" select="$currentSecondEditorList">
                                                ...
                                                <xsl:with-param name="authorCitationName" select="$currentSecondEditorCitationStyleName"
                                                ></xsl:with-param>
                                            </xsl:apply-templates>
                                        </xsl:element>
                                    </xsl:when>
            ...
            <xsl:when test="$is-first">
                <xsl:choose>
                    ...
                    <xsl:when test="string-length($authorString) = 0">
                        ...
                        <xsl:text disable-output-escaping="yes">&lt;/i&gt;&lt;/span&gt;</xsl:text>, </xsl:when>
                    <!--
                    ...
                    <xsl:otherwise>
                        <xsl:text disable-output-escaping="yes">&lt;span class="Italic"&gt;&lt;i&gt;</xsl:text>
                        <xsl:value-of select="$authorCitationName"></xsl:value-of>; <xsl:value-of select="$authorString"></xsl:value-of>,
