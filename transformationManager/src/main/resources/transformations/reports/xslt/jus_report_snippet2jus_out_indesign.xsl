					...
					<xsl:choose>
						<xsl:when  test="count($item-list/escidocItem:item/escidocMetadataRecords:md-records/escidocMetadataRecords:md-record[publication:publication/eterms:creator/person:person/dc:identifier = $currentAuthorId   and publication:publication/eterms:creator/@role=$authorRole]) &gt; 0">
							<xsl:element name="p">
								...
								<xsl:apply-templates select="$currentAuthorList" mode="sortedAuthorList">
					...
					<xsl:choose>
						<xsl:when test="count($item-list/escidocItem:item/escidocMetadataRecords:md-records/escidocMetadataRecords:md-record[publication:publication/eterms:creator/person:person/dc:identifier = $currentFirstEditorId and publication:publication/eterms:creator/@role=$editorRole and (publication:publication/@type = $genreCollectedEdition or publication:publication/@type = $genreMonograph or publication:publication/@type = $genreCommentary or publication:publication/@type = $genreHandbook or publication:publication/@type = $genreProceedings) or publication:publication/@type = $genreFestschrift]) &gt; 0">
							<xsl:element name="p">
								...
								<xsl:apply-templates select="$currentFirstEditorList" mode="sortedAuthorList">
					...
					<xsl:choose>
						<xsl:when  test="count($item-list/escidocItem:item/escidocMetadataRecords:md-records/escidocMetadataRecords:md-record[publication:publication/eterms:creator/person:person/dc:identifier = $currentSecondEditorId  and publication:publication/eterms:creator/@role=$editorRole   and (publication:publication/@type = $genreJournal   or publication:publication/@type = $genreSeries)]) &gt; 0">
							<xsl:element name="p">
								...
								<xsl:apply-templates select="$currentSecondEditorList" mode="sortedAuthorList">
		...
		<xsl:element name="publication">
			...
			<xsl:choose>
				<xsl:when test="contains($bibCitation, 'span class')">
					...
					<xsl:value-of disable-output-escaping="yes" select="$beforeOccurance"/>
					<xsl:element name="review-of">
						<xsl:attribute name="aid:cstyle" select="'italics'"/>
						<xsl:value-of disable-output-escaping="yes" select="$occurance"/>
					</xsl:element>
					<xsl:value-of disable-output-escaping="yes" select="$afterOccurance"/>
				</xsl:when>
				<xsl:otherwise>
