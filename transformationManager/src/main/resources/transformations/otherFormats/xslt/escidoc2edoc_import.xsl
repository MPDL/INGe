...
<!--   Transformations from eSciDoc PubItem to eDoc Item in   eDoc import schema: http://edoc.mpg.de/doc/schema/zim_transfer.xsd  Mapping: http://colab.mpdl.mpg.de/mediawiki/PubMan_Func_Spec_eSciDoc_To_eDoc_Mapping   Author: Vlad Makarenko (initial creation)   $Author: mfranke $ (last changed)  $Revision: 3183 $   $LastChangedDate: 2010-05-27 16:10:51 +0200 (Thu, 27 May 2010) $ -->
 ...
 xmlns:Util="https://pubman.mpdl.mpg.de/util-functions"
	...
	<xsl:template name="record">
		<xsl:element name="record">
			...
			<xsl:element name="genre">
				...
				<xsl:value-of select="  if (exists($gid))   then $gid  else error(  QName('http://www.escidoc.de/transformation', 'err:NoMappingForEnum' ),   concat ('No mapping escidoc to edoc for publication type: ', $esd )  )  " />
			</xsl:element>
			<!-- CORPORATEBODY -->
			<!-- NOT in the mapping!!! -->
			<!-- xsl:if test="count(eterms:creator/person:person)=0"><xsl:value-of select="  error(  QName('http://www.escidoc.de/transformation', 'err:NoCreators' ),   concat ('No creators in item: ', ../../../@objid )  )  " /></xsl:if-->
			...
			<!--  not relevant for MPIPL <xsl:call-template name="relation"/>-->
		</xsl:element>
	</xsl:template>
	<!-- METAMETADATA -->
	<xsl:template name="metametadata"></xsl:template>
	<!-- PUBLICATION -->
	<xsl:template name="publication">
		...
		<xsl:element name="publication">
			...
			<!-- DATES -->
			<xsl:variable name="dp" select="  func:coalesce((dcterms:issued, eterms:published-online))  "/>
			<!-- DATEACCEPTED -->
			<xsl:copy-of select="func:genElementIf((  @type='http://purl.org/eprint/type/Thesis',   'dateaccepted',   func:getDate(func:coalesce((dcterms:dateAccepted, $dp)))  ))"/>
			<!-- DATEPUBLISHED -->
			<!-- no datepublished for thesises-->
			<xsl:copy-of select="func:genElementIf((  $dp!='' and @type!='http://purl.org/eprint/type/Thesis',   'datepublished',   func:getDate($dp)  ))"/>
			<!-- ENDDATEOFEVENT -->
			<xsl:copy-of select="func:genElement((  'enddateofevent',   func:getDate(func:coalesce((event:event/eterms:end-date, event:event/eterms:start-date)))  ))"/>
			<!-- DATESUBMITTED -->
				...
				<xsl:if test="dc:title">
					<xsl:element name="inbook">
						...
						<xsl:variable name="cre_con">
							<!-- BOOKCREATORFN -->
							<xsl:copy-of select="func:genElement((  'bookcreatorfn',  func:screators(eterms:creator[@role='http://www.loc.gov/loc.terms/relators/AUT']/person:person)   ))"/>
							<!-- BOOKCONTRIBUTORFN -->
							<xsl:copy-of select="func:genElement((  'bookcontributorfn',  func:screators(eterms:creator[@role!='http://www.loc.gov/loc.terms/relators/AUT']/person:person)   ))"/>
						</xsl:variable>
						<xsl:if test="$cre_con!=''">
							<xsl:copy-of select="$cre_con" />
						</xsl:if>
						<!-- BOOKCORPORATEBODY -->
						<xsl:copy-of select="func:genElementIf((  $cre_con='',  'bookcorporatebody',  string-join(  (eterms:publishing-info/dc:publisher, eterms:publishing-info/eterms:place),  '&#xA;'  )  ))"/>
						<!-- EDITIONDESCRIPTION -->
			...
			<xsl:if test="@type='http://purl.org/escidoc/metadata/ves/publication-types/issue'">
				<xsl:if test="dc:title">
					<xsl:element name="inissue">
						...
						<!-- ISSUECONTRIBUTORFN -->
						<xsl:copy-of select="func:genElement((  'issuecontributorfn',   func:screators(eterms:creator[@role!='http://www.loc.gov/loc.terms/relators/AUT']/person:person)  ))"/>
						<!-- ISSUECORPORATEBODY -->
						<xsl:copy-of select="func:genElement((  'issuecorporatebody',  string-join(  ( eterms:publishing-info/dc:publisher, eterms:publishing-info/eterms:place ),  '&#xA;'  )   ))"/>
					</xsl:element>
				</xsl:if>
			</xsl:if>
			<!-- INJOUIRNAL -->
			...
			<xsl:if test="@type='http://purl.org/escidoc/metadata/ves/publication-types/proceedings'">
				<xsl:if test="dc:title">
					<xsl:element name="inproceedings">
						...
						<xsl:copy-of select="func:genElement((  'proceedingscontributorfn',   func:screators(eterms:creator[@role!='http://www.loc.gov/loc.terms/relators/AUT']/person:person)  ))"/>
					</xsl:element>
				</xsl:if>
			</xsl:if>
			<!-- INSERIES -->
				...
				<xsl:if test="dc:title">
					<xsl:element name="inseries">
						...
						<xsl:variable name="scfn">
							<!-- SERIESCONTRIBUTORFN -->
							<xsl:copy-of select="func:genElement((  'seriescontributorfn',   func:screators(eterms:creator[@role!='http://www.loc.gov/loc.terms/relators/AUT']/person:person)  ))"/>
						</xsl:variable>
						<xsl:if test="$scfn!=''">
							<xsl:copy-of select="$scfn" />
						</xsl:if>
						<!-- SERIESCORPORATEBODY -->
						<xsl:copy-of select="func:genElementIf((  $scfn='',  'seriescorporatebody',  string-join(  (eterms:publishing-info/dc:publisher, eterms:publishing-info/eterms:place),  '&#xA;'  )  ))"/>
					</xsl:element>
				</xsl:if>
			</xsl:if>
		</xsl:element>
	...
	<xsl:template name="content">
		<xsl:element name="content">
			...
			<!-- VERSIONCOMMENT -->
			<xsl:copy-of select="func:genElement((  'versioncomment',   ../../../ei:properties/prop:version[version:status='released']/version:comment  ))"/>
			<!-- DATEOFEVENT -->
			<xsl:copy-of select="func:genElement(('dateofevent', event:event/eterms:start-date))"/>
			<!-- DISCIPLINE -->
			<xsl:copy-of select="func:genElement((  'discipline',   string-join (dc:subject[.!=''], '; ')  ))"/>
			...
			<!--   MARKUPABSTRACT,  MARKUPTITLE,  MARKUPTYPE  are not in the MAPPING -->
			<!-- INVITATIONSTATUS -->
			<xsl:copy-of select="func:genElementIf((  event:event/eterms:invitation-status!='',  'invitationstatus',  if (event:event/eterms:invitation-status='invited') then 'invited' else 'notspec'  ))"/>
			<!-- NAMEOFEVENT -->
			<xsl:copy-of select="func:genElement(('nameofevent', event:event/dc:title))"/>
			<!--   NUMBEROFWORDS,  OS,  OSVERSION  are not in the MAPPING -->
			<!-- PLACEOFEVENT -->
			<xsl:copy-of select="func:genElement(('placeofevent', event:event/eterms:place))"/>
			<!-- PLATFORM is not in the MAPPING -->
			<!-- PUBSTATUS -->
			<xsl:copy-of select="func:genElementIf((  ../../../ei:properties/prop:public-status='released',  'pubstatus',   'published'  ))"/>
			<!-- PHYDESC -->
