...
<!--   Back transformation from the eSciDoc metadata profile v2 to v1   Author: vmakarenko (initial creation)   $Author: vmakarenko $ (last changed)  $Revision: $   $LastChangedDate: $ -->
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="publication:publication">
		...
		<xsl:element name="escidocMetadataProfile:publication" >
			...
			<xsl:attribute name="type" select="  if (exists($v1))   then $v1  else error(  QName('http://www.escidoc.de/transformation', 'err:NoMappingForEnum' ),   concat ('No mapping v2.0 to v1.0 for publication type: ', $v2, ', item id: ', ../../../@objid )  )  " />
			<xsl:apply-templates/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="eterms:creator" priority="999">
		...
		<xsl:element name="publication:creator" namespace="http://escidoc.mpg.de/metadataprofile/schema/0.1/publication">
			...
			<xsl:attribute name="role" select="  if (exists($v1))   then $v1  else error(  QName('http://www.escidoc.de/transformation', 'err:NoMappingForEnum' ),   concat ('No mapping v2.0 to v1.0 for creator role: ', $v2, ', item id: ', ../../../../@objid )  )  " />
			<xsl:apply-templates />
		</xsl:element>
	</xsl:template>
	...
	<xsl:template match="eterms:review-method" priority="999">
		...
		<xsl:element name="publication:review-method" namespace="http://escidoc.mpg.de/metadataprofile/schema/0.1/publication">
			<xsl:value-of select="  if (exists($v1))   then $v1  else error(  QName('http://www.escidoc.de/transformation', 'err:NoMappingForEnum' ),   concat ('No mapping v2.0 to v1.0 for review method: ', $v1, ', item id: ', ../../../@objid )  )  " />
	...
	<xsl:template match="source:source">
		<xsl:element name="publication:source" namespace="http://escidoc.mpg.de/metadataprofile/schema/0.1/publication">
			...
			<xsl:if test="@type">
				<xsl:variable name="v2" select="@type"/>
				<xsl:variable name="v1" select="$vm/publication-type/v2-to-v1/map[@v2=$v2]"/>
				<xsl:attribute name="type" select="  if (exists($v1))   then $v1  else error(  QName('http://www.escidoc.de/transformation', 'err:NoMappingForEnum' ),   concat ('No mapping v2.0 to v1.0 for source type: ', $v2, ', item id: ', ../../../../@objid)  )  " />
			</xsl:if>
			<xsl:apply-templates/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="dc:language">
		...
		<xsl:element name="{name()}">
			<xsl:copy-of select="@*[name() != 'xsi:type']"/>
			<xsl:if test="@*[name() = 'xsi:type']">
				<xsl:attribute name="xsi:type" select="  if (@xsi:type=$vm/language/@v2)   then $vm/language/@v1  else @xsi:type  "/>
				<xsl:value-of select="  if ($v1!='')  then $v1  else $vm/language/v2-to-v1/@default  "/>
			</xsl:if>
			<!-- skip duplicated value of the element -->
			<xsl:apply-templates select="*/*"/>
		</xsl:element>
	...
	<xsl:template match="eterms:content-category" priority="999">
		...
		<xsl:element name="file:content-category" namespace="http://escidoc.mpg.de/metadataprofile/schema/0.1/file">
			<xsl:value-of select="  if ($v1!='')  then $v1  else .  "/>
			<xsl:apply-templates select="*/*" />
		</xsl:element>
	</xsl:template>
	...
	<xsl:template match="eterms:degree" priority="999">
		...
		<xsl:element name="publication:degree" namespace="http://escidoc.mpg.de/metadataprofile/schema/0.1/publication">
			...
			<xsl:value-of select="  if (exists($v1))   then $v1  else error(  QName('http://www.escidoc.de/transformation', 'err:NoMappingForEnum' ),   concat ('No mapping v2.0 to v1.0 for academic degree: ', $v2, ', item id: ', ../../../@objid )  )  " />
			<!-- skip duplicated value of the element -->
			<xsl:apply-templates select="*/*"/>
		</xsl:element>
	</xsl:template>
