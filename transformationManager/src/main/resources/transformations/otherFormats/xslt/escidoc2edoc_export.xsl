...
<!-- Transformations from eSciDoc PubItem to eDoc Item in eDoc export schema Mapping: http://colab.mpdl.mpg.de/mediawiki/PubMan_Func_Spec_eSciDoc_To_eDoc_Mapping Author: Vlad Makarenko (initial creation) $Author: vmakarenko $ (last changed) $Revision: 3743 $ $LastChangedDate: 2010-12-03 10:42:56 +0100 (Fri, 03 Dec 2010) $ -->
 ...
 xmlns:Util="https://pubman.mpdl.mpg.de/util-functions"
	...
	<xsl:template name="record">
		<xsl:element name="record">
			...
			<!-- NOT in the mapping <xsl:call-template name="copyrights"/>-->
		</xsl:element>
	</xsl:template>
	<!--== METADATA ==-->
	...
	<xsl:template name="metametadata">
		<xsl:element name="metametadata">
			...
			<!-- NOT in the MAPPING!!! owner container -->
		</xsl:element>
	</xsl:template>
	<!--== COPYRIGHT ==-->
	...
	<xsl:template name="basic">
		...
		<xsl:element name="basic">
			...
			<!-- NOT IN THE MAPPING !!!<xsl:element name="corporatebody"></xsl:element>-->
			<!-- TITLE -->
			...
			<xsl:if test=" $dp!='' and @type!='http://purl.org/eprint/type/Thesis' ">
			...
			<xsl:if test="@type='http://purl.org/eprint/type/Thesis'">
				<xsl:copy-of select=" func:genElement(( 'dateaccepted', func:getDate(func:coalesce(($dp, dcterms:dateAccepted))) ))"/>
			</xsl:if>
			<!-- DATESUBMITTED -->
			...
			<xsl:if test="$s/@type='http://purl.org/escidoc/metadata/ves/publication-types/issue'">
				<xsl:if test="$s/dc:title">
					...
					<!-- ISSUECONTRIBUTORFN -->
					<xsl:copy-of select="func:genElement(( 'issuecontributorfn', func:screators($s/eterms:creator[@role!='http://www.loc.gov/loc.terms/relators/AUT']/person:person) ))"/>
					<!-- ISSUECORPORATEBODY -->
					<xsl:copy-of select="func:genElement(( 'issuecorporatebody', string-join( ( $s/eterms:publishing-info/dc:publisher, $s/eterms:publishing-info/eterms:place ), '&#xA;' ) ))"/>
				</xsl:if>
			</xsl:if>
			<!-- VOLUME -->
			...
			<!-- ENDDATEOFEVENT -->
			<xsl:copy-of select="func:genElementIf(( event:event/eterms:start-date!='' and event:event/eterms:end-date!='', 'dateofevent', func:coalesce((event:event/eterms:end-date, event:event/eterms:start-date)) ))"/>
			<!-- SOURCE stuff -->
			<!-- BOOK stuff -->
			<xsl:if test="$s/@type='http://purl.org/eprint/type/Book'">
				<xsl:if test="$s/dc:title">
					...
					<xsl:variable name="cre_con">
						<!-- BOOKCREATORFN -->
						<xsl:copy-of select="func:genElement(( 'bookcreatorfn', func:screators($s/eterms:creator[@role='http://www.loc.gov/loc.terms/relators/AUT']/person:person) ))"/>
						<!-- BOOKCONTRIBUTORFN -->
						<xsl:copy-of select="func:genElement(( 'bookcontributorfn', func:screators($s/eterms:creator[@role!='http://www.loc.gov/loc.terms/relators/AUT']/person:person) ))"/>
					</xsl:variable>
					<xsl:if test="$cre_con!=''">
						<xsl:copy-of select="$cre_con" />
					</xsl:if>
					<!-- BOOKCORPORATEBODY -->
					<xsl:copy-of select="func:genElementIf(( $cre_con='', 'bookcorporatebody', string-join( ($s/eterms:publishing-info/dc:publisher, $s/eterms:publishing-info/eterms:place), '&#xA;' ) ))"/>
				</xsl:if>
			</xsl:if>
			<!-- PROCEEDINGS stuff -->
			<xsl:if test="$s/@type='http://purl.org/escidoc/metadata/ves/publication-types/proceedings'">
				<xsl:if test="$s/dc:title">
					...
					<xsl:copy-of select="func:genElement(( 'proceedingscontributorfn', func:screators($s/eterms:creator[@role!='http://www.loc.gov/loc.terms/relators/AUT']/person:person) ))"/>
				</xsl:if>
			</xsl:if>
			<!-- SERIES stuff -->
			<xsl:if test="$s/@type='http://purl.org/escidoc/metadata/ves/publication-types/series'">
				<xsl:if test="$s/dc:title">
					...
					<xsl:variable name="scfn">
						<!-- SERIESCONTRIBUTORFN -->
						<xsl:copy-of select="func:genElement(( 'seriescontributorfn', func:screators($s/eterms:creator[@role!='http://www.loc.gov/loc.terms/relators/AUT']/person:person) ))"/>
					</xsl:variable>
					<xsl:if test="$scfn!=''">
						<xsl:copy-of select="$scfn" />
					</xsl:if>
					<!-- SERIESCORPORATEBODY -->
					<xsl:copy-of select="func:genElementIf(( $scfn='', 'seriescorporatebody', string-join( ($s/eterms:publishing-info/dc:publisher, $s/eterms:publishing-info/eterms:place), '&#xA;' ) ))"/>
				</xsl:if>
			</xsl:if>
			<!-- EDITIONDESCRIPTION -->
			<!-- TODO: -->
			<!-- specify uri for genres -->
			<xsl:copy-of select="func:genElementIf(( $s/@type=( 'http://purl.org/eprint/type/Book', 'handbook', 'festschrift', 'commentary', 'commentary' ), 'editiondescription', $s/eterms:publishing-info/eterms:edition ))"/>
			<!-- END of SOURCE stuff -->
			<!-- ABSTRACT -->
