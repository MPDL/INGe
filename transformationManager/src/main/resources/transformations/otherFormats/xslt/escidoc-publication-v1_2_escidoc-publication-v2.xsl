...
<!--   Transformation from the eSciDoc metadata profile v1 to v2   Author: vmakarenko (initial creation)   $Author$ (last changed)  $Revision$   $LastChangedDate$ -->
<xsl:stylesheet version="2.0"
	...
	<!-- xsl:variable name="ous"><map v1="ou_4001">ou_274892</map><map v1="ou_4002">ou_274893</map><map v1="ou_4005">ou_274894</map><map v1="ou_4003">ou_274895</map><map v1="ou_4007">ou_274896</map></xsl:variable -->
	<xsl:template match="/">
		<xsl:choose>
	...
	<xsl:template match="escidocMetadataProfile:publication">
		...
		<xsl:element name="pub:publication" namespace="http://purl.org/escidoc/metadata/profiles/0.1/publication">
			...
			<xsl:attribute name="type" select="  if (exists($v2))   then $v2  else error(  QName('http://www.escidoc.de/transformation', 'err:NoMappingForEnum' ),   concat ('No mapping v1.0 to v2.0 for publication type: ', $v1, ', item id: ', $objid)  )  " />
			<xsl:apply-templates/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="publication:creator | escidoc:creator" priority="999" >
		<xsl:variable name="objid" select="  if (namespace-uri()='http://escidoc.mpg.de/metadataprofile/schema/0.1/publication')   then ../../../@objid   else ../../../../@objid  "/>
		...
		<xsl:element name="eterms:creator">
			...
			<xsl:if test="$v2!=''">
				<xsl:attribute name="role" select="  if (exists($v2))   then $v2  else error(  QName('http://www.escidoc.de/transformation', 'err:NoMappingForEnum' ),   concat ('No mapping v1.0 to v2.0 for creator role: ', $v1, ', item id: ', $objid)  )  " />
			</xsl:if>
			<xsl:apply-templates />
		</xsl:element>
	</xsl:template>
	...
	<xsl:template match="escidoc:identifier" priority="999">
		<xsl:element name="dc:identifier">
			...
			<xsl:if test="@*[name() = 'xsi:type']">
				...
				<xsl:value-of select="  if (contains(@xsi:type, 'CONE'))  then concat('http://cone.mpdl.mpg.de/persons/resource/', substring-after(., 'urn:cone:'))  else .  "/>
			</xsl:if>
			<xsl:if test="not(@xsi:type)">
				<!-- xsl:variable name="ou" select="."/><xsl:value-of select="  if ($ous/map[@v1=$ou]) then $ous/map[@v1=$ou]  else .   "/ -->
				<xsl:value-of select="."/>
			</xsl:if>
	...
	<xsl:template match="publication:review-method">
		...
		<xsl:element name="eterms:review-method">
			<xsl:value-of select="  if (exists($v2))   then $v2  else error(  QName('http://www.escidoc.de/transformation', 'err:NoMappingForEnum' ),   concat ('No mapping v1.0 to v2.0 for review method: ', $v1, ', item id: ', $objid)  )  " />
	...
	<xsl:template match="publication:source | escidoc:source" priority="999">
		<xsl:element name="source:source">
			...
			<xsl:if test="@type">
				...
				<xsl:variable name="v2" select="$vm/publication-type/v1-to-v2/map[@v1=$v1]"/>
				<xsl:attribute name="type" select="  if (exists($v2))   then $v2  else error(  QName('http://www.escidoc.de/transformation', 'err:NoMappingForEnum' ),   concat ('No mapping v1.0 to v2.0 for source type: ', $v1, ', item id: ', $objid)  )  " />
			</xsl:if>
			<xsl:apply-templates/>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template match="publication:degree">
		...
		<xsl:element name="eterms:degree">
			...
			<xsl:value-of select="  if (exists($v2))   then $v2  else error(  QName('http://www.escidoc.de/transformation', 'err:NoMappingForEnum' ),   concat ('No mapping v1.0 to v2.0 for academic degree: ', $v1, ', item id: ', $objid)  )  " />
			<!-- skip duplicated value of the element -->
			<xsl:apply-templates select="*/*"/>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template match="file:content-category" priority="999">
		...
		<xsl:element name="eterms:content-category" namespace="http://purl.org/escidoc/metadata/terms/0.1/">
			<xsl:value-of select="  if ($v2!='')  then $v2  else .  "/>
			<xsl:apply-templates select="*/*" />
		</xsl:element>
	</xsl:template>
	...
	<!-- all escidoc: prefixes to the eterms:   Note: escidoc:identifier for person and organization has own processing   -->
