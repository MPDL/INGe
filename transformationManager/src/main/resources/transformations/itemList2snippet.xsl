...
<!--   Transformation from itemList to snippet format  Author: walter (initial creation)   $Author: walter $ (last changed)  $Revision: 5346 $   $LastChangedDate: $ -->
	...
	<xsl:template match="escidocItem:item/escidocItem:properties/prop:content-model-specific">
		<xsl:copy>
			...
			<dcterms:bibliographicCitation>
				...
				<xsl:value-of disable-output-escaping="yes" select="$citations[$item-position]"/>
