...
<xsl:stylesheet version="2.0"
	...
	<xsl:template name="createEntry">
		...
		<xsl:element name="pub:publication">
			...
			<xsl:for-each select="N[  $refType = 'Generic' and $Flavor = 'MPIGEM'  ]">
				<dc:identifier xsi:type="eterms:OTHER">
					<xsl:value-of select="." />
				</dc:identifier>
			</xsl:for-each>
			...
			<!-- früherer Abschnitt dateAccepted: <xsl:variable name="dateAccepted" select=" if ($dateCreated and $refType = 'Thesis') then $dateCreated else 
				'' "/><xsl:if test="$dateAccepted != ''"><dcterms:dateAccepted xsi:type="dcterms:W3CDTF"><xsl:value-of select="$dateAccepted"/></dcterms:dateAccepted></xsl:if> -->
			<!-- end of DATES -->
			...
			<xsl:element name="dc:title">
				<xsl:choose>
					...
					<!-- Ergänzung für MPI IPP für Altdatenimport aus Libero. Haben hin 
						und wieder ZS-Angaben auch bei Genre Conference Paper! -->
					<xsl:when test="J and $Flavor = 'IPP' and $refType = 'Conference Paper'">
						<xsl:value-of select="J" />
					</xsl:when>
	...
	<xsl:template name="createCreators">
		...
		<xsl:for-each select="A|E|Y|QUESTION|NUM_3">
			<xsl:variable name="currentCreatorPosition" select="position()" />
			<xsl:comment>
				-- Creator-Position: <xsl:value-of select="$currentCreatorPosition"/>
 --
			</xsl:comment>
			<xsl:if test="name(.)='A'">
				<xsl:choose>
					<xsl:when test="  $refType = (  'Generic',   'Book',   'Book Section',   'Conference Paper',   'Conference Proceedings',   'Electronic Article',   'Electronic Book',   'Journal Article',   'Magazine Article',   'Newspaper Article',   'Manuscript',   'Report',   'Thesis',  'Artwork',   'Classical Work',  'Courseware', 'Dataset', 'Blog', 'Film or Broadcast' )">
						...
						<xsl:call-template name="createCreator">
							...
							<xsl:with-param name="pos" select="count(../A[position() &lt; $currentCreatorPosition]) + 1" />
							<xsl:with-param name="gen" select="$gen" />
						</xsl:call-template>
					</xsl:when>
					<!-- INTERVIEW -->
					<xsl:when test="  $refType = (  'Interview'  )">
						...
						<xsl:call-template name="createCreator">
							<xsl:with-param name="role" select="$creator-ves/enum[.='interviewee']/@uri" />
							<xsl:with-param name="pos" select="count(../A[position() &lt; $currentCreatorPosition]) + 1" />
							<xsl:with-param name="gen" select="$gen" />
						</xsl:call-template>
					</xsl:when>
					...
					<!-- SOFTWARE -->
					<xsl:when test="  $refType = (  'Computer Program'  )">
						<xsl:comment>
						    -- Author --
						</xsl:comment>
						<xsl:call-template name="createCreator">
							<xsl:with-param name="role" select="$creator-ves/enum[.='developer']/@uri" />
							<xsl:with-param name="pos" select="count(../A[position() &lt; $currentCreatorPosition]) + 1" />
							<xsl:with-param name="gen" select="$gen" />
						</xsl:call-template>
					</xsl:when>
				...
				<xsl:choose>
					<xsl:when test="$refType='Generic'">
						...
						<xsl:call-template name="createCreator">
							...
							<xsl:with-param name="pos" select="count(../E[position() &lt; $currentCreatorPosition]) + 1" />
							<xsl:with-param name="gen" select="$gen" />
						</xsl:call-template>
					</xsl:when>
			...
			<xsl:if test="name(.)='E'">
				<xsl:choose>
					<xsl:when test="$refType = 'Interview' ">
						...
						<xsl:call-template name="createCreator">
							<xsl:with-param name="role" select="$creator-ves/enum[.='interviewer']/@uri" />
							<xsl:with-param name="pos" select="count(../E[position() &lt; $currentCreatorPosition]) + 1" />
							<xsl:with-param name="gen" select="$gen" />
						</xsl:call-template>
					</xsl:when>
			...
			<xsl:if test="name(.)='Y'">
				<xsl:choose>
					<xsl:when test="$refType='Generic' or ($refType='Conference Proceedings' and S)">
						...
						<xsl:call-template name="createCreator">
							<xsl:with-param name="role" select="$creator-ves/enum[.='contributor']/@uri" />
							<xsl:with-param name="pos" select="count(../Y[position() &lt; $currentCreatorPosition]) + 1" />
							<xsl:with-param name="gen" select="$gen" />
						</xsl:call-template>
					</xsl:when>
			...
			<xsl:if test="name(.)='QUESTION'">
				<xsl:if test="$refType = ('Book', 'Book Section', 'Edited Book')">
					...
					<xsl:call-template name="createCreator">
						...
						<xsl:with-param name="pos" select="count(../QUESTION[position() &lt; $currentCreatorPosition]) + 1" />
						<xsl:with-param name="gen" select="$gen" />
					</xsl:call-template>
				</xsl:if>
		...
		<xsl:choose>
			<xsl:when test="$CoNE = 'false'">
				<xsl:element name="person:person">
					<xsl:comment>
						-- CoNE==false -- $pos: <xsl:value-of select="$pos" /> --
					</xsl:comment>
					...
					<xsl:choose>
						<xsl:when test="not($isSource) and $Flavor != 'MPFI' and $pos=1">
					...
					<xsl:if test="$Flavor != ''">
						<xsl:variable name="publication-date">
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:choose>
										<xsl:when test="(gen = 'http://purl.org/escidoc/metadata/ves/publication-types/article' or gen = 'http://purl.org/escidoc/metadata/ves/publication-types/magazine-article') and exists(NUM_7) and NUM_7 != ''">
											<xsl:value-of select="NUM_7" />
										</xsl:when>
										<xsl:otherwise>
											<xsl:choose>
												<xsl:when test="(gen != 'http://purl.org/escidoc/metadata/ves/publication-types/article' or gen = 'http://purl.org/escidoc/metadata/ves/publication-types/magazine-article') and exists(NUM_8) and NUM_8 != ''">
													<xsl:value-of select="NUM_8" />
												</xsl:when>
											</xsl:choose>
										</xsl:otherwise>
			...
			<xsl:when test="$internal-managed">
				...
				<escidocComponents:component>
					...
					<mdr:md-records xmlns:escidocMetadataRecords="http://www.escidoc.de/schemas/metadatarecords/0.5">
						<mdr:md-record name="escidoc">
							<file:file xmlns:file="http://purl.org/escidoc/metadata/profiles/0.1/file"
