...
<xsl:stylesheet version="2.0"
		...
		<xsl:element name="source:source">
			...
			<xsl:element name="dc:title">
				<xsl:choose>
					...
					<!-- Ergänzung für MPI IPP für Altdatenimport aus Libero. Haben hin und wieder ZS-Angaben auch bei Genre Conference Paper! -->
	...
	<xsl:template name="createCreators">
		...
		<xsl:for-each select="A|E|Y|QUESTION|NUM_3">
			<xsl:if test="name(.)='A'">
				<xsl:choose>
					<xsl:when test="  $refType = (  'Generic',   'Book',   'Book Section',   'Conference Paper',   'Conference Proceedings',   'Electronic Article',   'Electronic Book',   'Journal Article',   'Magazine Article',   'Newspaper Article',   'Manuscript',   'Report',   'Thesis',  'Artwork',   'Classical Work',  'Courseware'  )">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<person:person>
					...
					<!--<xsl:if test="exists($cone-creator/cone/rdf:RDF/rdf:Description) and $Flavor = 'CAESAR'"><organization:organization><dc:title><xsl:text>External Organizations</xsl:text></dc:title><dc:identifier><xsl:text>ou_persistent22</xsl:text></dc:identifier></organization:organization></xsl:if> -->
			...
			<xsl:when test="$internal-managed">
				...
				<escidocComponents:component>
					<escidocComponents:properties
						...
						<prop:visibility>
							<xsl:choose>
								<xsl:when test="not($Flavor = 'CAESAR') and $oa">public</xsl:when>
								<xsl:when test="not($oa) and $Flavor = 'MPIMP' or $Flavor = 'MPIMPExt'">audience</xsl:when>
								<xsl:when test="$Flavor = 'CAESAR'">private</xsl:when>
