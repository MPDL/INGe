...
<!--   Transformations from BioMedCentral to eSciDoc PubItem   Author: Julia Kurt (initial creation)   $Author$ (last changed)  $Revision$   $LastChangedDate$ -->
<xsl:stylesheet version="2.0"
	...
	<xsl:template name="createItem">
		<xsl:element name="ei:item">
			<xsl:element name="ei:properties">
				...
				<!--<xsl:element name="srel:context"><xsl:attribute name="xlink:href" select="concat('/ir/context/', $context)"/></xsl:element>-->
		...
		<xsl:variable name="date">
			<xsl:if test="bmc:Year">
				...
				<xsl:if test="bmc:Month">
					<xsl:choose>
						<xsl:when test="bmc:Month='Jan'">
							<xsl:text>-01</xsl:text>
						</xsl:when>
						<xsl:when test="bmc:Month='Feb'">
							<xsl:text>-02</xsl:text>
						</xsl:when>
						<xsl:when test="bmc:Month='Mar'">
							<xsl:text>-03</xsl:text>
						</xsl:when>
						<xsl:when test="bmc:Month='Apr'">
							<xsl:text>-04</xsl:text>
						</xsl:when>
						<xsl:when test="bmc:Month='May'">
							<xsl:text>-05</xsl:text>
						</xsl:when>
						<xsl:when test="bmc:Month='Jun'">
							<xsl:text>-06</xsl:text>
						</xsl:when>
						<xsl:when test="bmc:Month='Jul'">
							<xsl:text>-07</xsl:text>
						</xsl:when>
						<xsl:when test="bmc:Month='Aug'">
							<xsl:text>-08</xsl:text>
						</xsl:when>
						<xsl:when test="bmc:Month='Sep'">
							<xsl:text>-09</xsl:text>
						</xsl:when>
						<xsl:when test="bmc:Month='Oct'">
							<xsl:text>-10</xsl:text>
						</xsl:when>
						<xsl:when test="bmc:Month='Nov'">
							<xsl:text>-11</xsl:text>
						</xsl:when>
						<xsl:when test="bmc:Month='Dec'">
	...
	<!-- TOTAL NO OF PAGES -->
	<!-- <xsl:template match="pm:page-range"><xsl:element name="eterms:total-number-of-pages"><xsl:value-of select="."/></xsl:element></xsl:template>-->
	<!-- ABSTRACT -->
	<!--TODO delete tags -->
