...
<!--   Transformations from PubMedCentral to eSciDoc PubItem   Author: Julia Kurt (initial creation)   $Author$ (last changed)  $Revision$   $LastChangedDate$ -->
			...
			<xsl:choose>
				<xsl:when test="../../../pm:aff[@id=$orgaRef]">
					...
					<xsl:element name="organization:organization">
						...
						<!-- address -->
						<!--<xsl:call-template name="createAdressLine"><xsl:with-param name="orgaRef" select="$orgaRef"/><xsl:with-param name="orgaPath" select="../../../aff[@id=$orgaRef]"/></xsl:call-template>-->
					</xsl:element>
				</xsl:when>
				<xsl:when test="../../pm:aff[@id=$orgaRef]">
	...
	<xsl:template name="createJournal">
		<xsl:element name="source:source">
			...
			<xsl:for-each select="../pm:journal-meta">
				<!-- <xsl:apply-templates select="pm:journal-title"/><xsl:apply-templates select="pm:journal-subtitle"/><xsl:apply-templates select="pm:trans-title"/><xsl:apply-templates select="pm:trans-subtitle"/><xsl:apply-templates select="pm:abbrev-journal-title"/>-->
				<!-- Fix by Christian Steiger, 2011-07-18 -->
				<xsl:apply-templates select=".//pm:journal-title[1]"/>
