...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="oaipmh:OAI-PMH/oaipmh:GetRecord/oaipmh:record/oaipmh:metadata">
		<html>
			<head>
				<style type="text/css">
					<!-- Basic style sheet to give sans-serif output -->body, h1, h2, h3, h4, table {font-family: sans-serif;}  body {font-size: x-small;}  h1 {font-size: x-large;}  h2 {font-size: large;}  h3 {font-size: medium;}  h4 {font-size: small; font-weight: normal;}  table {font-size: x-small;}
				</style>
			</head>
			<body>
				<xsl:for-each select="bmc:art">
					<xsl:for-each select="bmc:fm">
						...
						<xsl:apply-templates select="bmc:abs" />
						<!-- renders the article abstract -->
					</xsl:for-each>
					<xsl:apply-templates select="bmc:bdy" />
					<!-- renders the main body -->
					<xsl:for-each select="bmc:bm">
						<xsl:apply-templates select="bmc:ack" />
						<!-- Acknowledgements -->
						<xsl:apply-templates select="bmc:refgrp"/>
						<!-- References -->
						<xsl:apply-templates select="bmc:sec" />
						<!-- other matter -->
					</xsl:for-each>
				</xsl:for-each>
			</body>
	...
	<xsl:template name="render-tig">
		<a name="top">
			<xsl:apply-templates select="bmc:dochead" mode="tig"/>
		</a>
	...
	<xsl:template name="render-tig-bibl-details">
		...
		<xsl:if test="(bmc:aug/bmc:au[@ce = 'yes']) or (bmc:aug/bmc:au[@ca = 'yes'])">
			...
			<xsl:if test="bmc:aug/bmc:au[@ce='yes']">
		...
		<xsl:if test="bmc:volume[1]">
			...
			<xsl:if test="bmc:issue">(
				<xsl:value-of select="bmc:issue"/>)
			</xsl:if>
			<b>:</b>
		</xsl:if>
		<xsl:if test="bmc:fpage[1]">
			<xsl:value-of select="bmc:fpage"/>
			<xsl:if test="bmc:lpage[1] and bmc:fpage[1] != bmc:lpage[1]">-
				<xsl:value-of select="bmc:lpage"/>
			</xsl:if>
		</xsl:if>
		...
		<xsl:if test="boolean(bmc:note)">
			<br/>
			<br/>
			<xsl:apply-templates select="bmc:note" mode="tig"/>
		</xsl:if>
	</xsl:template>
	<xsl:template name="render-tig-emails">
		<br />
		<xsl:if test="bmc:aug/bmc:au/bmc:email">Email: 
			<xsl:call-template name="render-tig-emails-authors" />
		</xsl:if>
		<br />
	</xsl:template>
	<xsl:template name="render-tig-emails-authors">
		<xsl:for-each select="bmc:aug/bmc:au/bmc:email">
			...
			<xsl:if test="../bmc:mnm">&#160;
				<xsl:value-of select="../bmc:mnm"/>
			</xsl:if>
			<xsl:if test="../bmc:snm">&#160;
				<xsl:value-of select="../bmc:snm"/>
			</xsl:if>
			<xsl:if test="../bmc:cnm">
	...
	<xsl:template name="render-doi">
		<xsl:for-each select="bmc:xrefbib//bmc:pubid[@idtype='doi']">&#160;&#160;&#160;&#160;&#160;doi:
			<xsl:value-of select="."/>
		</xsl:for-each>
	</xsl:template>
				...
				<a name="{generate-id()}">
					<h2>
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:apply-templates select="bmc:st/bmc:p" />
							</xsl:otherwise>
						</xsl:choose>
					</h2>
				</a>
	...
	<xsl:template name="article-map">
		<xsl:for-each select="../bmc:bdy/bmc:sec | ../bmc:bdy//bmc:fig | ../bmc:bdy//bmc:tbl | ../bmc:bdy//bmc:suppl">
			<xsl:choose>
				...
				<xsl:when test="local-name()='fig'">
					<a href="#{generate-id()}">
						<xsl:apply-templates select="bmc:title/bmc:p"/>
					</a>
				</xsl:when>
				<xsl:when test="local-name()='tbl'">
					<a href="#{generate-id()}">
		...
		<xsl:for-each select="//bmc:refgrp">
	...
	<xsl:template match="bmc:refgrp">
		...
		<br />
		<xsl:choose>
			<xsl:when test="bmc:bibl[@rating='1']">
				<xsl:call-template name="render-ratings" />
			</xsl:when>
			<xsl:when test="bmc:bibl[@rating='2']">
				<xsl:call-template name="render-ratings" />
			</xsl:when>
		</xsl:choose>
	...
	<xsl:template match="bmc:bibl" mode="ref">
		<tr>
			...
			<td valign="top" colspan="2">
				<a name="{@id}" />
				<xsl:if test="child::bmc:note = child::node()[1]">
					<xsl:apply-templates select="bmc:note" />
					<br />
				</xsl:if>
	...
	<xsl:template name="bibl-body">
		...
		<xsl:if test="not(bmc:aug)">
			<xsl:apply-templates select="bmc:editor" mode="book"/>
		</xsl:if>
		<xsl:choose>
			<xsl:when test="bmc:publisher and not(bmc:url) and not(bmc:source)">
				<xsl:apply-templates select="bmc:title" mode="book"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="bmc:title" mode="ref"/>
			</xsl:otherwise>
		</xsl:choose>
		<xsl:if test="bmc:source[1] | bmc:edition[1] | bmc:editor[1] | bmc:publisher[1] | bmc:pubdate[1] | bmc:inpress[1] | bmc:volume[1] | bmc:issue[1] | bmc:fpage[1] | bmc:lpage[1] | bmc:xrefbib[1] | bmc:url[1]">
			<xsl:if test="bmc:title">
				<br />
			</xsl:if>
			<xsl:apply-templates select="bmc:url" mode="ref"/>
			<xsl:choose>
				<xsl:when test="bmc:publisher and not(bmc:url)">
					<!-- if we have a publisher but not a url then we assume reference is to a book -->
					<xsl:choose>
						<xsl:when test="bmc:title and not(bmc:source = 'PhD thesis')">
							<xsl:apply-templates select="bmc:source" mode="book"/>
						</xsl:when>
						<xsl:otherwise>
					...
					<xsl:if test="bmc:aug">
						<xsl:apply-templates select="bmc:editor" mode="ref"/>
					</xsl:if>
			...
			<xsl:if test="bmc:fpage[1] | bmc:lpage[1]">
				<xsl:apply-templates select="bmc:fpage" mode="ref"/>
				<xsl:apply-templates select="bmc:lpage" mode="ref"/>
				<xsl:text>. </xsl:text>
			</xsl:if>
	...
	<xsl:template match="bmc:editor" mode="ref">
		<xsl:text>Edited by: </xsl:text>
		<xsl:value-of select="."/>
		<xsl:text>. </xsl:text>
	</xsl:template>
	<xsl:template match="bmc:editor" mode="book">
		<xsl:value-of select="."/>
		<xsl:text> (Ed</xsl:text>
		<xsl:if test="contains(.,',') or contains(.,' and ')">s</xsl:if>
		<xsl:text>): </xsl:text>
	</xsl:template>
	<xsl:template match="bmc:publisher" mode="ref">
		<xsl:value-of select="."/>
		<xsl:text>; </xsl:text>
	</xsl:template>
	<xsl:template match="bmc:inpress" mode="ref">, in press
		<xsl:text>. </xsl:text>
	</xsl:template>
	...
	<xsl:template match="bmc:xrefbib" mode="ref">
		...
		<!-- if PubMed has a full text link then display that   but only if it is not a BMC article  -->
	...
	<xsl:template match="bmc:fpage" mode="ref">
		<xsl:value-of select="." />
	</xsl:template>
	<xsl:template match="bmc:lpage" mode="ref">-
		<xsl:value-of select="." />
	</xsl:template>
	<xsl:template name="render-ratings">Papers of particular interest have been highlighted as:
		<br/>&#8226; of special interest
		<br/>&#8226;&#8226; of outstanding interest
		<br/>
		<br/>
	</xsl:template>
	...
	<xsl:template match="bmc:tbl">
		<table width="100%">
			...
			<tr>
				<td>
					<xsl:for-each select="bmc:caption">
						<b>
							<xsl:apply-templates select="bmc:p" />
						</b>
					</xsl:for-each>
	...
	<xsl:template match="bmc:suppl">
		<table width="100%">
			...
			<tr>
				<td>
					<xsl:apply-templates select="bmc:text/bmc:p"/>
				</td>
			</tr>
			<tr>
				<td>
					<a href="{$base-url}{$suppl-url}{bmc:file/@name}">
						<xsl:apply-templates select="bmc:file/bmc:p"/>
					</a>
					<hr />
				</td>
			</tr>
	...
	<!-- common functions -->
	<xsl:template match="bmc:p">
		<xsl:apply-templates />
	</xsl:template>
	<xsl:template match="bmc:b">
		<b>
			<xsl:apply-templates />
		</b>
	</xsl:template>
	<xsl:template match="bmc:it">
		<i>
			<xsl:apply-templates />
		</i>
	</xsl:template>
	<xsl:template match="bmc:sub">
		<sub>
			<xsl:apply-templates />
		</sub>
	</xsl:template>
	<xsl:template match="bmc:sup">
		<sup>
			<xsl:apply-templates />
		</sup>
	</xsl:template>
	<xsl:template match="bmc:monospace">
		<font class="monospace">
			<xsl:apply-templates />
		</font>
	</xsl:template>
	<xsl:template match="bmc:a">
		<a>
	...
	<xsl:template match="bmc:ul">
		<u>
			<xsl:apply-templates />
		</u>
	</xsl:template>
	<xsl:template match="bmc:hr">
		<hr />
	</xsl:template>
	...
	<xsl:template match="bmc:day|bmc:year" mode="full">
		<xsl:value-of select="number(.)" />
	</xsl:template>
	<xsl:template match="bmc:day|bmc:year">
		<xsl:value-of select="number(.)" />
	</xsl:template>
	...
	<xsl:template match="bmc:abbrgrp">
		...
		<xsl:for-each select="bmc:abbr">
			<xsl:choose>
				...
				<xsl:when test="ancestor::bmc:bdy/../bmc:bm/bmc:refgrp/bmc:bibl[@id=concat('B', current()+1)]/@rating > 0">
					<xsl:choose>
						<xsl:when test="preceding-sibling::bmc:abbr[position()=2]=current()-2">-
							<xsl:call-template name="render-with-bullets"/>
	...
	<xsl:template match="bmc:figr">
		...
		<a href="#{generate-id(ancestor::bmc:art//bmc:fig[@id = $pos])}">
			<xsl:value-of select="."/>
		</a>
	</xsl:template>
	<xsl:template match="bmc:tblr">
		...
		<a href="#{generate-id(ancestor::bmc:art//bmc:tbl[@id = $pos])}">
			<xsl:value-of select="."/>
		</a>
	</xsl:template>
	<xsl:template match="bmc:supplr">
		...
		<a href="#{generate-id(ancestor::bmc:art//bmc:suppl[@id = $pos])}">
			<xsl:value-of select="."/>
		</a>
	</xsl:template>
</xsl:stylesheet>