...
<xsl:stylesheet version="2.0"
	...
	<xsl:param name="ou-url" select="'http://qa.pure.mpdl.mpg.de'"/>
	
	<xsl:variable name="ou-list" select="document(concat($ou-url, '/rest/ous/xml?from=0&amp;size=5000'))"></xsl:variable>
	...
	<xsl:template match="escidoc:position/rdf:Description/dc:identifier">
		...
		<xsl:if
			test="count($ou-list/root/records/record[normalize-space(name) = $ou]) &gt; 1">
			<xsl:comment>ERROR with "<xsl:value-of select="$ou"></xsl:value-of>" (found more than one entry)</xsl:comment>
		</xsl:if>
		<xsl:if
			test="not(exists($ou-list/root/records/record[normalize-space(name) = $ou]/objectId))">
			<xsl:comment>ERROR with "<xsl:value-of select="$ou"></xsl:value-of>" (NO ID FOUND)</xsl:comment>
		</xsl:if>
		<xsl:if
			test="$ou-list/root/records/record[normalize-space(name) = $ou]/objectId = ''">
			<xsl:comment>ERROR with "<xsl:value-of select="$ou"></xsl:value-of>" (ID EMPTY)</xsl:comment>
		</xsl:if>
		<dc:identifier>
			<xsl:value-of
				select="$ou-list/root/records/record[normalize-space(name) = $ou]/objectId"
			></xsl:value-of>
		</dc:identifier>
