...
<!--  Transformations from eSciDoc PubItem to OAI DC See mapping:  http://colab.mpdl.mpg.de/mediawiki/PubMan_Func_Spec_Dublin_Core_Mapping  Author: Vlad Makarenko (initial creation) $Author$ (last changed)  $Revision$ $LastChangedDate$  -->
		...
		<xsl:for-each select="//pub:publication">
			<xsl:element name="oai_dc:dc"   namespace="http://www.openarchives.org/OAI/2.0/oai_dc/"   >
				...
				<xsl:for-each select="eterms:creator">
					...
					<xsl:variable name="creatorType" select="  if ($role='author') then 'dc:creator'   else if ($role = ('advisor', 'contributor', 'transcriber', translator, 'honoree')) then 'dc:contributor'  else if (empty(following-sibling::*[@role=$author_uri]|preceding-sibling::*[@role=$author_uri])) then 'dc:creator'  else 'dc:contributor'  "/>
					<xsl:if test="person:person!=''">
						<xsl:element name="{$creatorType}">
							<xsl:value-of  select="string-join((person:person/eterms:family-name, person:person/eterms:given-name), ', ' ) " />
						</xsl:element>
