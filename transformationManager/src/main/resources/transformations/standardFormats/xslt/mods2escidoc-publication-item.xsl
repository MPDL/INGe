...
<!--   Transformations from a GDZ Item to eSciDoc PubItem   Author: Andreas Gros (initial creation)   $Author: agros $ (last changed)  $Revision: 0000 $   $LastChangedDate: 2009-09-15 18:09:23 +0200 (Di, 15 Sep 2009) $ -->
<xsl:stylesheet version="2.0"
 ...
 xmlns:util="https://pubman.mpdl.mpg.de/util-functions"
	...
	xmlns:prop="http://escidoc.de/core/01/properties/"  xsi:schemaLocation="http://www.loc.gov/mods/v3  http://www.loc.gov/standards/mods/v3/mods-3-3.xsd  http://www.loc.gov/METS/  http://www.loc.gov/standards/mets/version17/mets.v1-7.xsd"
	...
	<!-- Kristina has to create a corresponding context <xsl:param name="context" select="'escidoc:XXXXXX'"/>-->
	...
	<xsl:template match="mets:dmdSec">
		<xsl:choose>
			<!-- SOURCE -->
			<!--VOLUME-->
			<xsl:when test="@ID = 'DMDLOG_0001' and .//mods:partName = 'Band'"></xsl:when>
			<!--ISSUE-->
			<xsl:when test="matches(@ID, 'DMDLOG_\d+') and .//mods:partName = 'Heft'"></xsl:when>
			<!--CONTRIBUTIONS-->
			<xsl:otherwise>
							...
							<mdr:md-records>
								<mdr:md-record name="escidoc">
									<file:file
										...
										<!-- TODO: Filesize -->
										<dcterms:extent>
											<xsl:value-of select="util:getSize(concat($filename-prefix, $component-name))"/>
										</dcterms:extent>
									</file:file>
								</mdr:md-record>
							</mdr:md-records>
	...
	<xsl:template name="createEntry">
		...
		<xsl:element name="mdp:publication">
			...
			<!-- EVENT -->
			<!-- PAGES -->
			<!--<xsl:if test=".//mods:start"><xsl:element name="e:start-page"><xsl:value-of select=".//mods:start"/></xsl:element><xsl:if test=".//mods:end"><xsl:element name="e:end-page"><xsl:value-of select=".//mods:end"/></xsl:element></xsl:if></xsl:if>-->
			<xsl:choose>
