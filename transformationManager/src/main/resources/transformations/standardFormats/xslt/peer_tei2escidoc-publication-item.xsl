...
<!--   Transformations from TEI 2 to eSciDoc PubItem   See mapping: http://colab.mpdl.mpg.de/mediawiki/PubMan_Func_Spec_Submission/TEI_2_PubItem_Mapping   Author: Vlad Makarenko (initial creation)   $Author: vmakarenko $ (last changed)  $Revision: $   $LastChangedDate: $ -->
 ...
 xmlns:AuthorDecoder="https://pubman.mpdl.mpg.de/author-decoder-functions"
	...
	<xsl:template name="createEntry">
		...
		<xsl:element name="pub:publication">
			...
			<xsl:element name="dc:title">
				<xsl:value-of select="  if (exists($fDesc/t:titleStmt/t:title[@type='main'][1]))  then normalize-space($fDesc/t:titleStmt/t:title[@type='main'][1])  else if (exists($sDesc/t:biblStruct/t:analytic/t:title[@type='main'][1]))  then normalize-space($sDesc/t:biblStruct/t:analytic/t:title[@type='main'][1])  else error(QName('http://www.escidoc.de', 'err:NoTitleDefined' ), 'No title is defined for the item')  "/>
			</xsl:element>
			<!-- LANGUAGE -->
			...
			<!--ALTTITLE -->
			<xsl:for-each select="  $fDesc/t:titleStmt/t:title[empty(@type) or @type!='main' or (@type='main' and position()>1) ]  |  $sDesc/t:biblStruct/t:analytic/t:title[empty(@type) or @type!='main' or (@type='main' and position()>1)]  ">
			...
			<xsl:for-each select="$dateMap/m/@key">
				...
				<xsl:if test="exists($date) or exists($change)">
					<xsl:element name="{$dateMap/m[@key=$dateType]}">
						...
						<xsl:value-of select="  if (  translate($date/@when, '-', '') >   translate($change/@when, '-', '')  )  then $date/@when  else $change/@when  "/>
					</xsl:element>
				</xsl:if>
			</xsl:for-each>
			<!-- SOURCE -->
			...
			<xsl:if test="$subjCount>0">
				<xsl:element name="dcterms:subject">
					<xsl:for-each select="$pDesc/t:textClass/t:keywords/t:list/t:item/t:term">
						<xsl:value-of select="  concat(  normalize-space(.),  if ($subjCount>1 and (position()!=last())) then ', ' else ''  )   "/>
					</xsl:for-each>
				</xsl:element>
			</xsl:if>
			<!-- LOCATION -->
	...
	<xsl:template name="createSource">
		...
		<xsl:element name="source:source">
			<!-- SOURCE GENRE -->
			<xsl:attribute name="type">
				<xsl:value-of select="  if ($monogr/t:title/@level='j')   then $genre-ves/enum[.='journal']/@uri   else if ($monogr/t:title/@level='s')   then 'series'  else error(QName('http://www.escidoc.de', 'err:UnknownSourceGenre' ), 'Unknown or empty Source genre: ', $monogr/t:title/@level)  "/>
			</xsl:attribute>
			<!-- SOURCE TITLE -->
			<xsl:variable name="title" select="  if (count($monogr/t:title[@type='main'])=0)   then $monogr/t:title[1]  else $monogr/t:title[@type='main']  "/>
			<xsl:element name="dc:title">
	...
	<xsl:template name="createPerson">
		<xsl:element name="person:person">
			...
			<xsl:for-each select="t:affiliation">
				<xsl:variable name="orgName" select="  if (exists(t:orgName) and (exists(t:orgName[@type='department']) or exists(t:orgName[@type='institution'])))  then string-join( (t:orgName[@type='department'], t:orgName[@type='institution']), ', ')  else if (exists(.))  then .  else 'External Organization'  "/>
					...
					<xsl:if test="exists($addr)">
						...
						<xsl:variable name="emails">
							<xsl:for-each select="t:email">
								<xsl:value-of select="  concat(  normalize-space(.),  if ($emlCount>1 and (position()!=last())) then ', ' else ''  )   "/>
							</xsl:for-each>
						</xsl:variable>
						<eterms:address>
							<xsl:variable name="country"   select="if(count($addr/addr/t:country > 0))   then $addr/addr/t:country[1]  else ''"/>
							<xsl:value-of select="  replace(  normalize-space(  string-join(  (  $addr/t:addrLine  , if (exists($addr/t:postCode)) then concat(', ',$addr/t:postCode) else ''  , $addr/t:settlement  , if (exists($addr/t:country)) then concat(', ', $country) else ''   , if (exists($emails)) then concat(' (', $emails, ')') else ''   )  , ' '  )  ),  '\s+,',  ','   )"/>
						</eterms:address>
					</xsl:if>
					<dc:identifier>
		...
		<xsl:for-each select="$idents">
			...
			<xsl:if test="not($isISSN) or ($isISSN and $ident!='N')">
				<xsl:element name="dc:identifier">
					...
					<xsl:value-of select="  if ($isISSN)  then concat('(', $ident/@type, ')', $ident)  else $ident  "/>
				</xsl:element>
			</xsl:if>
		</xsl:for-each>
	</xsl:template>
