...
<!--   Transformations from TEI 2 to eSciDoc PubItem   See mapping: http://colab.mpdl.mpg.de/mediawiki/Zeitschrift_Naturforschung#Mapping_ZfNTei2PubItem  Author: Friederike Kleinfercher (initial creation)  -->
<xsl:stylesheet version="2.0"
		...
		<xsl:element name="pub:publication">
			...
			<xsl:element name="dcterms:subject">
				<xsl:for-each select="$pDesc/t:textClass/t:keywords/t:list/t:item/t:term">
					<xsl:value-of select="  concat(.,  if ($subjCount>1 and (position()!=last())) then ', ' else ''  )   "/>
				</xsl:for-each>
			</xsl:element>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template name="createOrganization">
		<xsl:variable name="orgName" select="  if (exists(t:affiliation/t:orgName) and (exists(t:affiliation/t:orgName[@type='department']) or exists(t:affiliation/t:orgName[@type='institution'])  or exists(t:affiliation/t:orgName[@type='laboratory'])))  then string-join( (t:affiliation/t:orgName[@type='department'], t:affiliation/t:orgName[@type='institution'], t:affiliation/t:orgName[@type='laboratory']), ', ')  else 'External Organization'  "/>
		<organization:organization>
			...
			<xsl:if test="exists($addr)">
				<eterms:address>
					<xsl:value-of select="  string-join(  (  $addr/t:postBox  , $addr/t:addrLine  , $addr/t:postCode  , $addr/t:settlement  , $addr/t:country   )  , ', '  )   "/>
				</eterms:address>
			</xsl:if>
			<dc:identifier>
