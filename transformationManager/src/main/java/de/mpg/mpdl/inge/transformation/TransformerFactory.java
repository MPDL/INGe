...
public class TransformerFactory {
	...
	public enum FORMAT
	{
			...
			ESCIDOC_ITEM_V2_XML,
			ESCIDOC_ITEM_V1_XML,
			ESCIDOC_ITEMLIST_V1_XML,
			ESCIDOC_ITEMLIST_V2_XML,
			ESCIDOC_ITEMLIST_V3_XML,
			ESCIDOC_COMPONENT_XML,
			ESCIDOC_ITEM_VO,
			COINS_STRING,
			DOI_METADATA_XML,
			ZIM_XML,
			...
			JUS_HTML_XML,
			DC_XML,
			HTML_METATAGS_DC_XML,
			HTML_METATAGS_HIGHWIRE_PRESS_CIT_XML,
			OAI_DC,
			MODS_XML,
			PEER_TEI_XML,
			ZFN_TEI_XML,
	...
	public static void main(String[] arg){
		...
		try {
			...
			Transformer t = TransformerFactory.newInstance(FORMAT.ESCIDOC_ITEM_V3_XML, FORMAT.ESCIDOC_ITEM_V1_XML);
			StringWriter wr = new StringWriter();
			
			t.transform(new TransformerStreamSource(new FileInputStream("C:\\Users\\haarlae1\\Downloads\\export_escidoc_xml_v13.xml")), new TransformerStreamResult(wr));
			System.out.println(wr.toString());
			
			
			
			/*
			Transformer t = TransformerFactory.newInstance(FORMAT.ENDNOTE_STRING, FORMAT.ESCIDOC_ITEMLIST_V3_XML);
			StringWriter wr = new StringWriter();
			
			t.transform(new TransformerStreamSource(new FileInputStream("C:\\Users\\haarlae1\\Documents\\Pubman\\Import files\\Endnote\\endnote_bgc.xml")), new TransformerStreamResult(wr));
			System.out.println(wr.toString());
			
			
			Transformer bmcTrt = TransformerFactory.newInstance(FORMAT.BMC_XML, FORMAT.ESCIDOC_ITEMLIST_V2);
			StringWriter bmcWr = new StringWriter();
			
			
			String content = new Scanner(new File("C:\\Users\\haarlae1\\Documents\\Pubman\\Import files\\TestdatenBMC\\Testdaten Markus\\1752-1947-5-391.xml")).useDelimiter("\\Z").next();
			System.out.println(content);
			
			bmcTrt.transform(new XmlSource(new StreamSource(new FileInputStream("C:\\Users\\haarlae1\\Documents\\Pubman\\Import files\\TestdatenBMC\\Testdaten Markus\\1752-1947-5-391.xml"))), new XmlResult(new StreamResult(bmcWr)));
			...
			Transformer bmcTrt = TransformerFactory.newInstance(FORMAT.MARC_21_STRING, FORMAT.ESCIDOC_ITEMLIST_V2_XML);
			...
			bmcTrt.transform(new StringSource(new FileInputStream("C:\\Users\\haarlae1\\Documents\\Pubman\\Import files\\Marc21\\CUP_MPG_10014090_marc_record_19Jan2012_Update.mrc")), new TransformerStreamResult(new StreamResult(bmcWr)));
			System.out.println(bmcWr.toString());
				*/
			...
			handler.setResult(new StreamResult(resWr));
			
			handler.startDocument();
			handler.startPrefixMapping("", "http://test");
			...
			handler.endElement("123", "collection", "coll:collection");
			handler.endDocument();
