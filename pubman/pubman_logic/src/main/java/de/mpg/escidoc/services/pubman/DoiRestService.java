...
public class DoiRestService {
	...
	public static String getNewDoi(PubItemVO pubItem) throws Exception {
		if (logger.isDebugEnabled()) {
			logger.debug("Getting new DOI for ["
		...
		try {
			...
			String doiMetadataXml = new String(transformation.transform(
					itemXml.getBytes("UTF-8"),
					DoiMetadataTransformation.ESCIDOC_ITEM_FORMAT,
					DoiMetadataTransformation.DOI_ITEM_FORMAT, "escidoc"),
					"UTF-8");
			...
			Credentials defaultcreds = new UsernamePasswordCredentials(
					PropertyReader.getProperty("escidoc.doi.service.user"),
			...
			PutMethod putMethod = new PutMethod(
					PropertyReader.getProperty("escidoc.doi.service.url")
							+ PropertyReader
									.getProperty("escidoc.doi.service.create.url")
							+ queryParams);
			...
			if (statusCode != 201) {
				...
				logger.error("Error occured, when contacting DOxI. StatusCode="
						+ statusCode);
			...
			logger.error("Error getting new DOI for ["
					+ pubItem.getVersion().getObjectId() + "]", e);
			throw new Exception("Error getting new DOI for ["
					+ pubItem.getVersion().getObjectId() + "]", e);
		}

		if (logger.isDebugEnabled()) {
			logger.debug("Successfully retrieved new DOI for ["
					+ pubItem.getVersion().getObjectId() + "]");
		}
			...
			if (file.getVisibility() == FileVO.Visibility.PUBLIC
					&& ("http://purl.org/escidoc/metadata/ves/content-categories/any-fulltext"
							...
							|| "http://purl.org/escidoc/metadata/ves/content-categories/post-print"
									.equals(file.getContentCategory()) || "http://purl.org/escidoc/metadata/ves/content-categories/publisher-version"
								.equals(file.getContentCategory()))) {
				doiReady = true;
			}
		}
