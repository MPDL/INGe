...
function addFullItemFunctions() {
	...
	$('.fullItem').find('.itemInfoSwitch').each(function(i,ele){$(ele).click(function(){$(this).parents('.listItem').find('.shortView').slideToggle('normal'); resizeToFull();});});
...
//	$('.fileUploadBtn').each(function(i, elem){ if($(elem).parents('.fileSection').find('.fileInput').val() == ''){ $(elem).parents('.fileSection').find('.fileUploadBtn').attr('disabled','disabled');}; });
	...
	$('.showMultipleAuthors').each(function(i,elem){ if($(elem).parents('.itemBlock').find("input[type='hidden'][value='showPermanent']").length > 0) {$(elem).hide(); $(elem).parents('.itemBlock').find('.multipleAuthors').show(); $(elem).parents('.itemBlock').find('.firstCreator').removeClass('noTopBorder'); resizeToFull();}; });
	
	$('.checkAll').click(function(){ $(this).parents('.itemLine').find('.checkboxDoubleGroup').find(':checkbox').attr('checked','true'); $(this).parents('.itemLine').find('.checkboxDoubleGroup').find('span:hidden').show(); $(this).parents('.itemLine').find('.checkboxDoubleGroup').find('.showMoreCheckboxes').hide(); resizeToFull(); });
	$('.showMoreCheckboxes').click(function(){ $(this).hide(); $(this).siblings().show(); resizeToFull(); });
