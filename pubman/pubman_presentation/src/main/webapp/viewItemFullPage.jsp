...
<jsp:root version="2.1" xmlns:f="http://java.sun.com/jsf/core"
	...
	<f:view locale="#{InternationalizationHelper.userLocale}">
		...
		<meta name="description"
			content="#{ViewItemFull.pubItem.descriptionMetaTag}"></meta>
		...
		<tr:form usesUpload="true">
			<div class="full wrapper"><h:inputHidden id="offset"></h:inputHidden>
			...
			<div class="clear">
			<div class="headerSection"><jsp:directive.include
				file="header/Breadcrumb.jspf" />

			<div id="contentSkipLinkAnchor" class="clear headLine"><!-- Headline starts here -->
			...
			<h:panelGroup
				...
				<div class="free_area0 sub"><!-- content menu upper line starts here -->
				<h:outputLink id="lnkLinkForActionsView" styleClass="free_area0"
				...
				</h:outputLink> <h:outputText styleClass="free_area0"
					value="#{lbl.ViewItemFull_lblItemActions}"
					rendered="#{ViewItemSessionBean.subMenu == 'ACTIONS'}" /> <h:outputText
					styleClass="seperator void" /> <h:outputLink
					id="lnkLinkForExportView" styleClass="free_area0"
					...
					rendered="#{ViewItemSessionBean.subMenu == 'EXPORT' and !ViewItemFull.isStateWithdrawn}" />
				<!-- content menu upper line ends here --></div>
				<h:panelGroup layout="block" styleClass="free_area0 sub action"
					...
					<h:commandLink id="lnkSubmit" action="#{ViewItemFull.submitItem}"
						value="#{lbl.actionMenu_lnkSubmit}"
						rendered="#{ViewItemFull.canSubmit}" />
					<h:panelGroup styleClass="seperator"
						rendered="#{ViewItemFull.canSubmit}" />

					<h:commandLink id="lnkRelease" action="#{ViewItemFull.submitItem}"
						value="#{lbl.actionMenu_lnkRelease}"
						rendered="#{ViewItemFull.canRelease}" />
					<h:panelGroup styleClass="seperator"
						rendered="#{ViewItemFull.canRelease}" />

					<h:commandLink id="lnkAccept" action="#{ViewItemFull.acceptItem}"
						value="#{lbl.actionMenu_lnkAccept}"
						rendered="#{ViewItemFull.canAccept}" />
					<h:panelGroup styleClass="seperator"
						rendered="#{ViewItemFull.canAccept}" />
					<h:commandLink id="lnkRevise" action="#{ViewItemFull.reviseItem}"
					...
					<h:commandLink id="lnkModify" action="#{ViewItemFull.modifyItem}"
					...
					<h:commandLink id="lnkRemoveFromYearbook" styleClass="free_area0"
				...
				<h:panelGroup layout="block" styleClass="free_area0 sub action"
					...
					<h:selectOneMenu id="selEXPORTFORMAT"
						value="#{ExportItemsSessionBean.exportFormatName}"
						styleClass="xLarge_select replace"
					...
					<h:selectOneMenu id="selFILEFORMAT"
						value="#{ExportItemsSessionBean.fileFormat}"
						styleClass="medium_select replace"
				...
				<h:panelGroup layout="block" styleClass="free_area0 sub action"
					...
					<h:commandLink id="btnExportEMail" styleClass="free_area0"
						value="#{lbl.export_btEMail}" action="#{ViewItemFull.exportEmail}" />
					<!-- content menu lower line ends here -->
				</h:panelGroup>

				<!-- content menu ends here -->
			</h:panelGroup> <h:panelGroup layout="block" styleClass="subHeader"
				rendered="#{ViewItemFull.isStateInRevision}">
				<h:outputText
					value="#{msg.ViewItemFull_inRevision} #{ViewItemFull.pubItem.version.lastMessage}"
					rendered="#{ViewItemFull.canShowLastMessage}" />
				<h:outputText
					value="#{msg.ViewItemFull_inRevision} #{lbl.lbl_noEntry}"
					rendered="#{!ViewItemFull.canShowLastMessage}" />
			</h:panelGroup> <h:panelGroup layout="block" styleClass="subHeader"
				rendered="#{ViewItemFull.isStateSubmitted}">
				<h:outputText
					value="#{msg.ViewItemFull_submitted} #{ViewItemFull.pubItem.version.lastMessage}"
					rendered="#{ViewItemFull.canShowLastMessage}" />
				<h:outputText
					value="#{msg.ViewItemFull_submitted} #{lbl.lbl_noEntry}"
					rendered="#{!ViewItemFull.canShowLastMessage}" />
			</h:panelGroup>
			<div class="subHeader"><!-- Subheadline starts here --> <!-- JSF messages -->
			<h:messages styleClass="singleMessage" errorClass="messageError"
				...
				<h:messages errorClass="messageError" warnClass="messageWarn"
					...
					rendered="#{ViewItemFull.hasMessages}" />
			...
			</h:panelGroup> <!-- Survey link --> <h:panelGroup layout="block"
				style="margin-top:1em;"
				rendered="#{not empty HomePage.surveyUrl}">
				<div class="xHuge_area2_p6 messageArea"><span
					class="half_area0">
				<h2><h:outputText value="#{HomePage.surveyTitle}" /></h2>
				</span> <span class="huge_area0"> <h:outputText
					value="#{HomePage.surveyText}" /> </span> <span class="free_area0">
				<div class="medium_area2_p6 small_marginLExcl"><h:outputLink
					styleClass="activeButton" value="#{HomePage.surveyUrl}"
					title="User Survey" target="_blank">
			...
			<h:panelGroup layout="block" styleClass="full_area0"
				...
				<h:commandLink id="lnkManageAudience" styleClass="free_area0"
					action="#{AudienceBean.manageAudience}"
					rendered="#{ViewItemFull.canManageAudience}">
					<h:outputText value="#{lbl.AudiencePage}" />
				</h:commandLink>
				<h:panelGroup styleClass="seperator" rendered="false" /> <h:outputLink
				...
				<h:commandLink id="lnkViewItemLogPage" styleClass="free_area0"
				...
				<h:commandLink id="lnkViewItemFull_btnItemStatistics"
					styleClass="free_area0" action="#{ViewItemFull.showStatistics}"
					rendered="#{ViewItemFull.canShowStatistics}">
					<h:outputText value="#{lbl.ViewItemFull_btnItemStatistics}" />
				</h:commandLink> <h:panelGroup styleClass="seperator"
					rendered="#{ViewItemFull.canShowRevisions}" />
				<h:commandLink id="lnkViewItemFull_btnItemRevisions"
				...
				<h:commandLink id="lnkViewItemFull_btnItemVersions"
					styleClass="free_area0" action="#{ViewItemFull.showReleaseHistory}"
					rendered="#{ViewItemFull.canShowReleaseHistory}">
				...
				</h:outputLink> <h:panelGroup styleClass="seperator" /> </span></div>
				<div class="full_area0 itemHeader"><h:panelGroup
					styleClass="xLarge_area0 endline blockHeader">
									&#160;	
								</h:panelGroup> <h:panelGroup styleClass="seperator" /> <h:panelGroup
					styleClass="free_area0_p8 endline itemHeadline">
					<b><h:outputText
				...
				<h:panelGroup layout="block"
					styleClass="medium_area0_p4 statusArea">
					<h:panelGroup
						styleClass="big_imgArea xSmall_marginLExcl withdrawnItem"
						rendered="#{ViewItemFull.isStateWithdrawn}" />
					<h:panelGroup
						styleClass="big_imgArea xSmall_marginLExcl pendingItem"
						rendered="#{ViewItemFull.isStatePending}" />
					<h:panelGroup
						styleClass="big_imgArea xSmall_marginLExcl submittedItem"
						rendered="#{ViewItemFull.isStateSubmitted}" />
					<h:panelGroup
				...
				<h:panelGroup layout="block"
					...
					<div class="free_area0 itemBlockContent endline"><b
						class="xLarge_area0_p8 endline labelLine clear"> <h:outputText
						value="#{lbl.ViewItem_lblModeratorContact}" /><span
						class="noDisplay">: </span> </b> <span
						class="xHuge_area0 xTiny_marginLExcl endline"> <h:outputLink
			...
			</h:panelGroup> <!-- end: content section --></div>
			</div>
