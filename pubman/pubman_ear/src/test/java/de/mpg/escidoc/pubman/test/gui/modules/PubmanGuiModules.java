...
public abstract class PubmanGuiModules extends SeleneseTestCase {
  ...
  /**
   * Properties from config file "selenium.properties" Includes all testing relevant data like user
   * accounts etc.
   */
  ...
  public PubmanGuiModules() throws IOException {
    InputStream instream =
        PubmanGuiModules.class.getClassLoader().getResource("selenium.properties").openStream();
  ...
  public PubmanItem createPubItem(ItemType type) {
    ...
    switch (type) {
      case Other:
        ...
        item.addEvent(event);
        details =
            new PubmanItemDetails("de", "2000", "2001", "2002", "2003", "2004", "2005", "123",
                "tableOfContent", ReviewType.INTERNAL, IdentifierType.ESCIDOC, "escidoc:12345");
        item.addDetails(details);
        persOrg =
            new PubmanItemPersonOrganizations(null, false, null, null, CreatorRole.AUTHOR,
                "firstName", "lastName", "orgaName", "orgaAdress");
        item.addPersonOrganisation(persOrg);
        source =
            new PubmanItemSource(SourceGenre.BOOK, "testbuch", CreatorRole.ARTIST,
                CreatorType.Person, "familyName", "givenName", "orgaName", "orgaAddress", "volume",
                "123", "publisher", "place", IdentifierType.ESCIDOC, "escidoc:65432", "edition",
                "issue", "1", "4231", "222");
  ...
  public void fillFileSection() {
    typeAndCheck("fileUploads:0:inpFile",
    ...
    selenium.waitForPageToLoad(MAX_PAGELOAD_TIMEOUT);
    typeAndCheck("fileUploads:1:inpAddFileFromUrl",
        "http://edoc.mpg.de/get.epl?fid=60832&did=376632&ver=0");
  ...
  public void fillSourceSection() {
    ...
    typeAndCheck("editSource:0:iterSourceAlternativeTitle:0:inpSourceAlternativeTitle",
        "Test alternative title 1");
    selenium.click("editSource:0:iterSourceAlternativeTitle:0:btnAddSourceAlternativeTitle");
    typeAndCheck("editSource:0:iterSourceAlternativeTitle:1:inpSourceAlternativeTitle",
        "Test alternative title 2");
    ...
    typeAndCheck(
        "editSource:0:iterSourceGroupCreator:0:iterSourceCreatorPersonAffiliation:0:inpSourceCreatorPersonAffiliationName",
        "MPG");
    selenium
        .click("editSource:0:iterSourceGroupCreator:0:iterSourceCreatorPersonAffiliation:0:btnAddSourceCreatorPersonAffiliation");
    typeAndCheck(
        "editSource:0:iterSourceGroupCreator:0:iterSourceCreatorPersonAffiliation:1:inpSourceCreatorPersonAffiliationName",
        "External");
    ...
    selenium.select("editSource:0:iterSourceDetailsIdentifier:0:selSourceDetailIdentifier",
        "label=ISI");
    typeAndCheck("editSource:0:iterSourceDetailsIdentifier:0:inpSourceDetailIdentifier",
        "asdfasdfasdf");
    selenium.click("editSource:0:iterSourceDetailsIdentifier:0:btnAddSourceDetailIdentifier");
    selenium.select("editSource:0:iterSourceDetailsIdentifier:1:selSourceDetailIdentifier",
        "label=PMC");
    typeAndCheck("editSource:0:iterSourceDetailsIdentifier:1:inpSourceDetailIdentifier",
        "asdfasdfasdf");
  ...
  public void doEasySubmissionStep1(PubmanItem item) {
    for (int second = 0;; second++) {
      if (second >= 60)
        fail("timeout");
      try {
        if (selenium.isElementPresent("easySubmission:easySubmissionStep1Manual:selGenre"))
          break;
      } catch (Exception e) {
      }
      try {
    ...
    selenium
        .select("easySubmission:easySubmissionStep1Manual:selGenre", "value=" + item.getGenre());
    selenium.waitForPageToLoad(MAX_PAGELOAD_TIMEOUT);
    typeAndCheck("easySubmission:easySubmissionStep1Manual:inpItemMetadataTitle", item
        .getBasicList().get(0).title);
    for (int i = 0; i < item.getFileList().size(); i++) {
      typeAndCheck(
          "easySubmission:easySubmissionStep1Manual:fileUploads:" + i + ":inpFileUploaded", item
              .getFileList().get(i).uploadUrl);
      selenium
          .click("easySubmission:easySubmissionStep1Manual:fileUploads:" + i + ":btnUploadFile");
      selenium.waitForPageToLoad(MAX_PAGELOAD_TIMEOUT);
      selenium.select("easySubmission:easySubmissionStep1Manual:fileUploads:" + i
          + ":selContentCategory", "value="
          + item.getFileList().get(i).contentCategory.toString().toLowerCase());
      typeAndCheck("easySubmission:easySubmissionStep1Manual:fileUploads:" + i
          + ":inpComponentDescription", item.getFileList().get(i).description);
      typeAndCheck("easySubmission:easySubmissionStep1Manual:fileUploads:" + i
          + ":inpComponentFileDefaultMetadataRights", item.getFileList().get(i).copyrightStatement);
    ...
    for (int i = 0; i < item.getLocatorsList().size(); i++) {
      typeAndCheck("easySubmission:easySubmissionStep1Manual:locatorUploads:" + i
          + ":inpLocatorLocator1", item.getLocatorsList().get(i).uploadUrl);
      selenium.click("easySubmission:easySubmissionStep1Manual:locatorUploads:" + i
          + ":lblEasySubmissionSaveLocator");
      selenium.waitForPageToLoad(MAX_PAGELOAD_TIMEOUT);
      selenium.click("easySubmission:easySubmissionStep1Manual:locatorUploads:" + i
          + ":selLocatorFileVisibility:0");
      typeAndCheck("easySubmission:easySubmissionStep1Manual:locatorUploads:" + i
          + ":inpLocatorFileDescription", item.getLocatorsList().get(i).description);
      typeAndCheck("easySubmission:easySubmissionStep1Manual:locatorUploads:" + i
          + ":inpLocatorFileDefaultMetadataRights",
          item.getLocatorsList().get(i).copyrightStatement);
      typeAndCheck("easySubmission:easySubmissionStep1Manual:locatorUploads:" + i
          + ":locatorLicenseDate", item.getLocatorsList().get(i).copyrightDate);
      // if we have more data, open another section
      if ((i + 1) < item.getLocatorsList().size()) {
        selenium.click("easySubmission:easySubmissionStep1Manual:locatorUploads:" + i
            + ":btnEasySubmissionAddLocator");
        selenium.waitForPageToLoad(MAX_PAGELOAD_TIMEOUT);
      }
    }
  }
  ...
  public void doEasySubmissionStep2(PubmanItem item) {
    if (item.getPersonOrganizationList().get(0).creatorRole != null) {
      selenium
          .select(
              "easySubmission:easySubmissionStep2Manual:iterCreatorOrganisationAuthors:0:selCreatorRoleString",
    ...
    for (int i = 0; i < item.getPersonOrganizationList().size(); i++) {
      if (item.getPersonOrganizationList().get(i).creatorType != null) {
        selenium.click("easySubmission:easySubmissionStep2Manual:iterCreatorCollection:" + i
            + ":selCreatorTypeString:"
            + item.getPersonOrganizationList().get(i).creatorType.ordinal());
      }
      typeAndCheck("easySubmission:easySubmissionStep2Manual:iterCreatorOrganisationAuthors:" + i
          + ":inpcreator_persons_person_family_name_optional", item.getPersonOrganizationList()
          .get(i).lastName);
      typeAndCheck("easySubmission:easySubmissionStep2Manual:iterCreatorOrganisationAuthors:" + i
          + ":inppersons_person_given_name_optional",
          item.getPersonOrganizationList().get(i).firstName);

      typeAndCheck("easySubmission:easySubmissionStep2Manual:iterCreatorOrganisationAuthors:" + i
          + ":inppersons_person_ous_optional", ""
      ...
      for (int u = 0; u < item.getPersonOrganizationList().get(i).getOrganizationList().size(); u++) {
        typeAndCheck("easySubmission:easySubmissionStep2Manual:iterCreatorOrganisation:" + i
            + ":inporganizations_organization_name", item.getPersonOrganizationList().get(i)
            .getOrganizationList().get(u).orgaName);
        typeAndCheck("easySubmission:easySubmissionStep2Manual:iterCreatorOrganisation:" + i
            + ":inporganizations_organization_address", item.getPersonOrganizationList().get(i)
            .getOrganizationList().get(u).orgaAddress);
        // if we have more data, open another section
        if ((u + 1) < item.getPersonOrganizationList().get(i).getOrganizationList().size()) {
          selenium.click("easySubmission:easySubmissionStep2Manual:iterCreatorCollection:" + i
              + ":iterCreatorPersonOrganisation:" + u + ":btnPersonOrganisationManagerAddObject");
          selenium.waitForPageToLoad(MAX_PAGELOAD_TIMEOUT);
        }
      }
      // if we have more data, open another section
    ...
    typeAndCheck("easySubmission:easySubmissionStep2Manual:inpFreeKeywords", item.getContentList()
        .get(0).keywords);
    // typeAndCheck("easySubmission:easySubmissionStep2Manual:inpSubjectValue",
    // item.getContentList().get(0).ddcSubject);
    typeAndCheck("easySubmission:easySubmissionStep2Manual:inpAbstract",
        item.getContentList().get(0).contentAbstract);
  }

  public void doEasySubmissionStep3(PubmanItem item) {
    typeAndCheck("easySubmission:easySubmissionStep3Manual:txtDatePublishedInPrint", item
        .getDetailsList().get(0).datePublishedInPrint);
    typeAndCheck("easySubmission:easySubmissionStep3Manual:txtDatePublishedOnline", item
        .getDetailsList().get(0).datePublishedOnline);
    typeAndCheck("easySubmission:easySubmissionStep3Manual:txtDateAccepted", item.getDetailsList()
        .get(0).dateAccepted);
    typeAndCheck("easySubmission:easySubmissionStep3Manual:txtDateSubmitted", item.getDetailsList()
        .get(0).dateSubmitted);
    typeAndCheck("easySubmission:easySubmissionStep3Manual:txtDateModified", item.getDetailsList()
        .get(0).dateModified);
    typeAndCheck("easySubmission:easySubmissionStep3Manual:txtDateCreated", item.getDetailsList()
        .get(0).dateCreated);
    selenium.select("easySubmission:easySubmissionStep3Manual:selSourceGenre", "value="
        + item.getSourceList().get(0).genre.toString());
    typeAndCheck("easySubmission:easySubmissionStep3Manual:inpSourceTitle", item.getSourceList()
        .get(0).title);
    ...
    typeAndCheck("easySubmission:easySubmissionStep3Manual:inpSourceDetailsStartPage", item
        .getSourceList().get(0).startPage);
    typeAndCheck("easySubmission:easySubmissionStep3Manual:inpSourceDetailsEndPage", item
        .getSourceList().get(0).endPage);
  }
  ...
  public void full_Submission_testing_submit(PubmanItem item) {
    ...
    for (int second = 0;; second++) {
      if (second >= 60)
        fail("timeout");
      try {
        if (selenium.isElementPresent("cboGenre"))
          break;
      } catch (Exception e) {
      }
      try {
    ...
    selenium.waitForPageToLoad(MAX_PAGELOAD_TIMEOUT);
    selenium.type("inputTitleText", item.getBasicList().get(0).title
        + "full submission paper submit");
    selenium.select("iterCreatorOrganisationAuthors:0:selCreatorRoleString", "label=Editor");
    selenium
        .mouseDown("iterCreatorOrganisationAuthors:0:inpcreator_persons_person_family_name_optional");
    selenium.keyPress(
        "iterCreatorOrganisationAuthors:0:inpcreator_persons_person_family_name_optional", "\\109");
    selenium.keyPress(
        "iterCreatorOrganisationAuthors:0:inpcreator_persons_person_family_name_optional", "\\97");
    try {
      Thread.sleep(1000);
    } catch (InterruptedException e) {
      e.printStackTrace();
    ...
    selenium.waitForPageToLoad(MAX_PAGELOAD_TIMEOUT);
    verifyTrue(selenium
        .isTextPresent("If genre is not equal to \"Series\" or \"Journal\" or \"Other\" or \"Manuscript\" at least one date has to be provided."));
    ...
    selenium.waitForPageToLoad(MAX_PAGELOAD_TIMEOUT);
    selenium.type("//div[@id='content']/div[2]/div[1]/div[2]/div/div[3]/span/textarea",
        "a test case with MD");
    selenium.click("lnkSave");
    selenium.waitForPageToLoad(MAX_PAGELOAD_TIMEOUT);
