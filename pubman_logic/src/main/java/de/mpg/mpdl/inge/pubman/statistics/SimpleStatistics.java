...
public class SimpleStatistics implements PubItemSimpleStatistics {
  ...
  // private void logPubItemAction(PubItemVO pubItem, String ip, String userAgent, ItemAction
  // action,
  ...
  // exportFormatParam
  // .setParamValue(new StatisticReportRecordStringParamValueVO(exportFormat.getName()));
  ...
  // exportFileFormatParam.setParamValue(new StatisticReportRecordStringParamValueVO(
  ...
  // List<CreatorVO> tempCreatorList = pubItem.getMetadata().getCreators();
  ...
  // for (int listSize = 0; listSize < creator.getPerson().getOrganizations()
  // .size(); listSize++) {
  // tempOrganizationList.add(creator.getPerson().getOrganizations().get(listSize));
  // }
  // for (int j = 0; j < tempOrganizationList.size(); j++) {
  // // if the organization is not in the list already, put it in.
  ...
  // if (file.getStorage() != Storage.EXTERNAL_URL
  // && file.getVisibility() == Visibility.PUBLIC) {
  // return true;
  // }
  // }
  // }
