...
public class PubItemBatchServiceImpl implements PubItemBatchService {
   ...
   */
  @Override
  public Map<String, Exception> changeReviewMethod(Map<String, Date> pubItemsMap,
    ...
    if (reviewMethodOld != null && reviewMethodNew != null
        && !reviewMethodOld.equals(reviewMethodNew)) {
      for (String itemId : pubItemsMap.keySet()) {
        try {
          ItemVersionVO pubItemVO = this.pubItemService.get(itemId, authenticationToken);
          ...
          logger.error(
              "Could not replace review method for item " + itemId + " due to a technical error");
          messageMap.put(itemId,
              new Exception("Review method have not been replaced due to a technical error"));
          throw e;
        } catch (AuthenticationException e) {
          logger.error(
              "Could not replace review method for item " + itemId + " due authentication error");
          messageMap.put(itemId,
              new Exception("Review method have not been replaced due to a authentication error"));
          throw e;
        } catch (AuthorizationException e) {
          logger.error(
              "Could not replace review method for item " + itemId + " due authentication error");
   ...
   */
  @Override
  public Map<String, Exception> changeSourceGenre(Map<String, Date> pubItemsMap,
