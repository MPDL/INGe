...
public interface PubItemSimpleStatistics
{
    ...
    public static final String REPORTDEFINITION_NUMBER_OF_ITEM_RETRIEVALS_ALL_USERS = "select month, year, sum(requests) as itemRequests from _escidocaggdef1_object_statistics where (object_id = {object_id} OR object_id LIKE {object_id} || ':%') and handler = 'de.escidoc.core.om.service.ItemHandler' and request='retrieve' group by month, year;";
    public static final String REPORTDEFINITION_FILE_DOWNLOADS_PER_ITEM_ALL_USERS = "select parent_object_id as itemId, sum(requests) as fileRequests from _escidocaggdef1_object_statistics where (parent_object_id = {object_id} OR parent_object_id LIKE {object_id} || ':%')  and handler = 'de.escidoc.core.om.service.ItemHandler' and request ILIKE 'retrieveContent' group by parent_object_id;";
    public static final String REPORTDEFINITION_FILE_DOWNLOADS_PER_FILE_ALL_USERS = "select month, year, sum(requests) as fileRequests from _escidocaggdef1_object_statistics where (object_id = {object_id} OR object_id LIKE {object_id} || ':%')  and handler = 'de.escidoc.core.om.service.ItemHandler' and request ILIKE 'retrieveContent' group by month, year;";
    public static final String REPORTDEFINITION_NUMBER_OF_ITEM_RETRIEVALS_ANONYMOUS = "select month, year, sum(requests) as itemRequests from _escidocaggdef1_object_statistics where (object_id = {object_id} OR object_id LIKE {object_id} || ':%')  and handler = 'de.escidoc.core.om.service.ItemHandler' and request='retrieve' and user_id='' group by month, year;";
    ...
    public static final String REPORTDEFINITION_FILE_DOWNLOADS_PER_FILE_ANONYMOUS = "select month, year, sum(requests) as fileRequests from _escidocaggdef1_object_statistics where (object_id = {object_id} OR object_id LIKE {object_id} || ':%') and handler = 'de.escidoc.core.om.service.ItemHandler' and request ILIKE 'retrieveContent' and user_id='' group by month, year;";
