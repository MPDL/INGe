...
public class TestSearchAndBrowse {
  ...
  private void displayRecords(SearchRetrieveResponseType searchResult) throws Exception {
    /*
     * RecordType[] records = searchResult.getRecords().getRecord(); if (records != null) { for (int
     * i = 0; i < records.length; ++i) { StringOrXmlFragment data = records[i].getRecordData();
     * MessageElement[] messages = data.get_any(); for (int j = 0; j < messages.length; ++j) {
  ...
  /**
   * Test method for
   * {@link gov.loc.www.zing.srw.service.ExplainPort#explainOperation(gov.loc.www.zing.srw.ExplainRequestType)}
   * .
   */
  @Test
  public void explainPlan() throws Exception {
    ...
    ExplainResponseType explainResponse =
        ServiceLocator.getExplainHandler("escidoc_all").explainOperation(explainRequest);
  ...
  /**
   * Test method for
   * {@link gov.loc.www.zing.srw.service.SRWPort#searchRetrieveOperation(gov.loc.www.zing.srw.SearchRetrieveRequestType)}
   * .
   */
  @Test
