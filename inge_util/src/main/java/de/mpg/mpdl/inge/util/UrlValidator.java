...
public class UrlValidator {
    ...
    /**
     * Validates if the given target URL is safe for redirection.
     * Throws an IllegalArgumentException if the target is invalid.
     *
        ...
        try {
            ...
            for (String allowedBase : allowedBaseUrls) {
                try {
                    ...
                    if (targetUrl.getProtocol().equalsIgnoreCase(allowedUrl.getProtocol()) &&
                        targetHost.equalsIgnoreCase(allowedUrl.getHost())) {
    ...
    /**
     * Validates if the given target URL is safe for redirection.
     * It compares the target URL against the default target configured in properties.
     *
     * @param target The URL to validate.
     * @return true if the target is considered safe, false otherwise.
     * @deprecated Use {@link #validate(String)} instead to get detailed error information via Exception.
     */
    @Deprecated
