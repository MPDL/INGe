package de.mpg.mpdl.inge.model.xmltransforming.statistics;

public class UserAgentAnalyser {
  ...
  // This method relies on the preceding one to get the version number. It then truncates it to the
  // numDigits first characters.
  ...
  // This method is a Java utility and doesn't really hold any logic. It takes three Strings as
  // parameters and returns an array of 3 strings. This array is used to return the right label for
  // the right verbosity.
  ...
  // This method detects if the userAgent provided is a Bot or Not. In case it is, it will return
  // the name of the Bot. Bots are considered as being Browsers in my terminology. They belong in
  // the OS "Bot". They hold only two levels of verbosity: 0 and 1 are not verbose, 2 is verbose,
  // including the version number. This method might be out of date as I am testing this code on a
  // system that doesn't record bots hits.
  ...
  /*
   * Here we go with a big piece: The detection of the OS. The code is mostly focused on Windows for
   * two reasons:
   * 
   * 1. It is the most encountered OS, see here for details. 2. The version numbers are a little
   * screwed as the OS is represented by two names (Win and Windows) and the version numbers for NT
   * by two different representations: XP and NT 5.1 for example.
   */
    ...
    if ((pos = userAgent.indexOf("Windows-NT")) > -1) {
      ...
      if (res == null) {
        if ((pos = userAgent.indexOf("Windows")) > -1) {
          res =
              getArray("Win", "<B>Win?</B>", "<B>Win?" + getVersionNumber(userAgent, pos + 7)
                  + "</B>");
        } else if ((pos = userAgent.indexOf("Win")) > -1) {
          res =
              getArray("Win", "<B>Win?</B>", "<B>Win?" + getVersionNumber(userAgent, pos + 3)
                  + "</B>");
        } else
      ...
      res =
          getArray("SonyEricsson", "SonyEricsson",
              "SonyEricsson" + getVersionNumber(userAgent, pos + 12));
      ...
      res =
          getArray("BlackBerry", "BlackBerry", "BlackBerry" + getVersionNumber(userAgent, pos + 10));
      ...
      res =
          getArray("Nintendo Wii", "Nintendo Wii",
              "Nintendo Wii" + getVersionNumber(userAgent, pos + 10));
    } else
  ...
  /*
   * Here is the main part: The detection of the Browser. Some interesting tricks:
   * 
   * 1. As Opera has the good idea of including a version of IE in its User-agent, it has to be
   * tested before IE. 2. Gecko stands for the rendering engine of Mozilla. It has several
   * packagings: Camino & Chimera for the Mac, Galeon for *nix and Phoenix, Firebird, and Firefox
   * for the generic versions from mozilla.org. Additionally, Netscape in its versions greater than
   * 6 also uses Gecko. 3. KHTML stands for the KDE renderer. It is packaged as Konqueror for Linux
   * and Safari for the Mac. 4. As most of the User-Agents begin with Mozilla/X.0, it is very
   * hazardous to detect browsers that does rely on this version number to actually describe
   * themselves. Among them is Netscape Communicator that hopefully is finally dying.
   */
  ...
  public static String[] getBrowser(String userAgent) {
    ...
    if ((pos = userAgent.indexOf("Lotus-Notes/")) > -1) {
      res =
          getArray("LotusNotes", "LotusNotes", "LotusNotes" + getVersionNumber(userAgent, pos + 12));
    } else if ((pos = userAgent.indexOf("Opera")) > -1) {
      res =
          getArray("Opera", "Opera" + getFirstVersionNumber(userAgent, pos + 5, 1), "Opera"
      ...
      res =
          getArray("KHTML", "KHTML(Chrome)", "KHTML(Chrome" + getVersionNumber(userAgent, pos + 6)
              + ")");
    } else if ((pos = userAgent.indexOf("Safari/")) > -1) {
      res =
          getArray("KHTML", "KHTML(Safari)", "KHTML(Safari" + getVersionNumber(userAgent, pos + 6)
              + ")");
    } else if ((pos = userAgent.indexOf("Konqueror/")) > -1) {
      res =
          getArray("KHTML", "KHTML(Konqueror)",
              "KHTML(Konqueror" + getVersionNumber(userAgent, pos + 9) + ")");
    ...
    if (userAgent.indexOf("Mozilla/4.") == 0 && userAgent.indexOf("Mozilla/4.0") < 0
      ...
      res =
          getArray("Communicator", "Communicator",
              "Communicator" + getVersionNumber(userAgent, pos + 8));
    } else
