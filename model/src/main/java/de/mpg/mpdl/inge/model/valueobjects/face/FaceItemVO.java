...
package de.mpg.mpdl.inge.model.valueobjects.face;
...
public class FaceItemVO extends ItemVO {
  ...
  public FaceItemVO() {
    try {
      this.setContentModel(PropertyReader
          .getProperty("escidoc.framework_access.content-model.id.faceitem"));
    } catch (Exception e) {
  ...
  /**
   * Getter for face-metadata. This only works if the forst metadata set of the item is of type
   * face-metadata.
   * 
  ...
  /**
   * Setter for face-metadata. Overrrides the first metadata set of the item. If there is no
   * metadata set, it will be added.
   * 
