...
public class StatisticLoggerBean implements StatisticLogger {
  private final static String PROPERTY_GEOIP_FILE_LOCATION =
  ...
  private void getStandardIPLookupService(File geopIpCopyFile) {
    ...
    try {
      ...
      InputStream is =
          ResourceUtil.getResourceAsStream("statistics/GeoIP.dat",
              StatisticLoggerBean.class.getClassLoader());
      ...
      logger
          .info("Geo IP Country functionality is activated. For city functionality please provide GeoLiteCity.db and add location to pubman.properties.");
    } catch (Exception e) {
      logger
          .error(
  ...
  public void logNewUser(String sessionId, String ip, String userAgent, String referer,
    ...
    if (browsers.length > 0) {
      browserParam.setParamValue(new StatisticReportRecordStringParamValueVO(
  ...
  public void logItemAction(String sessionId, String ip, String userAgent, ItemVO item,
    ...
    logger.debug("Logging new action " + action.toString() + "for item"
    ...
    itemIdParam.setParamValue(new StatisticReportRecordStringParamValueVO(item.getVersion()
        .getObjectId()));
    ...
    contextParam.setParamValue(new StatisticReportRecordStringParamValueVO(item.getContext()
        .getObjectId()));
    ...
    itemStatusParam.setParamValue(new StatisticReportRecordStringParamValueVO(item
        .getPublicStatus().toString()));
  ...
  public void logOrgAction(String sessionId, String ip, String orgId, boolean loggedIn,
    ...
    StatisticReportRecordParamVO actionParam = new StatisticReportRecordParamVO();
    actionParam.setName("action");
    actionParam.setParamValue(new StatisticReportRecordStringParamValueVO(
        "organization-retrieve-item"));
    paramList.add(actionParam);
