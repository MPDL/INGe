...
public class JibXTocDivMarshaller implements IMarshaller, IUnmarshaller, IAliasable {
  ...
  public void marshal(Object obj, IMarshallingContext ictx) throws JiBXException {
    ...
    if (tocDivVO.getId() != null)
      ctx.attribute(0, DIV_ID_ATTRIBUTE_NAME, tocDivVO.getId());
    if (tocDivVO.getLabel() != null)
      ctx.attribute(0, DIV_LABEL_ATTRIBUTE_NAME, tocDivVO.getLabel());
    if (tocDivVO.getOrder() != -1)
      ctx.attribute(0, DIV_ORDER_ATTRIBUTE_NAME, tocDivVO.getOrder());
    if (tocDivVO.getOrderLabel() != null)
      ctx.attribute(0, DIV_ORDERLABEL_ATTRIBUTE_NAME, tocDivVO.getOrderLabel());
    if (tocDivVO.getType() != null)
      ctx.attribute(0, DIV_TYPE_ATTRIBUTE_NAME, tocDivVO.getType());
    if (tocDivVO.getVisible() != null)
      ctx.attribute(0, DIV_VISIBLE_ATTRIBUTE_NAME, tocDivVO.getVisible());
    ...
    if (tocDivVO.getTocPtrList() != null) {
      ...
      for (TocPtrVO tocPtrVO : tocDivVO.getTocPtrList()) {
        ...
        if (tocPtrVO.getId() != null)
          ctx.attribute(0, PTR_ID_ATTRIBUTE_NAME, tocPtrVO.getId());
        ctx.attribute(0, PTR_LOCTYPE_ATTRIBUTE_NAME, tocPtrVO.getLoctype());
        if (tocPtrVO.getMimetype() != null)
          ctx.attribute(0, PTR_MIMETYPE_ATTRIBUTE_NAME, tocPtrVO.getMimetype());
        if (tocPtrVO.getUse() != null)
          ctx.attribute(0, PTR_USE_ATTRIBUTE_NAME, tocPtrVO.getUse());
        if (tocPtrVO.getLinkRef() != null)
          ctx.attribute(xlinkNsPos, PTR_REFLINK_ATTRIBUTE_NAME, tocPtrVO.getLinkRef());
        if (tocPtrVO.getLinkTitle() != null)
          ctx.attribute(xlinkNsPos, PTR_REFTITLE_ATTRIBUTE_NAME, tocPtrVO.getLinkTitle());
        if (tocPtrVO.getLinkType() != null)
          ctx.attribute(xlinkNsPos, PTR_REFTYPE_ATTRIBUTE_NAME, tocPtrVO.getLinkType());
        ctx.closeStartEmpty();
     ...
     * 
     * // loop through all entries in hashmap Iterator<MetadataSetVO> iter = list.iterator();
     * boolean first = true; while (iter.hasNext()) { MetadataSetVO entry = (MetadataSetVO)
     * iter.next(); ctx.startTagAttributes(m_index, RECORD_ELEMENT_NAME);
     * 
     * logger.debug("m_index: " + m_index);
     * 
     * if (first) { ctx.attribute(0, NAME_ATTRIBUTE_NAME, "escidoc"); } ctx.closeStartContent(); if
     * (entry instanceof IMarshallable) { ((IMarshallable) entry).marshal(ctx); ctx.endTag(m_index,
     * RECORD_ELEMENT_NAME); } else { throw new JiBXException("Mapped value is not marshallable (" +
     * entry.getClass().getSimpleName() + ")"); } first = false; }
     * 
     * // finish with end tag for container element ctx.endTag(m_index, m_name);
     */
