    ...
    try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
      ...
      for (YesNo fieldCommercial : YesNo.values()) {
        ...
        for (YesNo fieldDerivatives : YesNo.values()) {
          ...
          String licenceUrl =
              ccUrl + "&field_commercial=" + fieldCommercial.toString() + "&field_derivatives="
                  + fieldDerivatives.toString() + "&field_jurisdiction="
                  + fieldJurisdiction.toString() + "&lang=de_DE";
          System.out.println(licenceUrl);
          HttpGet method = new HttpGet(licenceUrl);
//          ProxyHelper.executeMethod(httpClient, method);
          try (var response = httpClient.execute(method)) {
          ...
          if (200 == response.getStatusLine().getStatusCode()) {
            ...
            String codeToCopy = extractCode(EntityUtils.toString(response.getEntity()));
            ...
            if (urlMatcher.find()) {
              ...
              HttpGet method2 = new HttpGet(url);
//              ProxyHelper.executeMethod(httpClient, method2);
              String page;
              try (var response2 = httpClient.execute(method2)) {
                page = EntityUtils.toString(response2.getEntity());
              }
  ...
  private static List<LocalizedTripleObject> extractLanguages(String page, String baseURL)
    ...
    Pattern pattern =
        Pattern
            .compile("<a\\s+href=\"./([^\"]+)\"\\s+title=\"([^\"]+)\"\\s+hreflang=\"([^\"]+)\"\\s+rel=\"alternate nofollow\"\\s+(xml:)?lang=\"([^\"]+)\">");
...
//      ProxyHelper.executeMethod(httpClient, method);
      String translation;
      try (CloseableHttpClient httpClient = HttpClients.createDefault(); var response = httpClient.execute(method)) {
        translation = EntityUtils.toString(response.getEntity());
      }
   ...
   * @throws IOException
   */
  private static String extractCode(String page) throws IOException {
    if (page.contains("<textarea id=\"codetocopy\"")) {
      ...
      System.out.println("codetocopy not found");
      return null;
    }
  }
