...
public class TestQueryParser {
  ...
  /*
   * TODO Single Quotes at the end or in the middle of a word are currently accepted by the parser,
   * but shouldn't be. Parser has to be changed
   * 
   * @Test public void testQueryParserWithClosingQuote() throws Exception { Reader sr = new
   * StringReader("test suche\"");; QueryParser qp = new QueryParser(sr); try { String cqlQuery =
   ...
   * }
   * 
   * @Test public void testQueryParserWithMiddleQuote() throws Exception { Reader sr = new
   * StringReader("test su\"che");; QueryParser qp = new QueryParser(sr); try { String cqlQuery =
   * qp.parse(); logger.info(cqlQuery); fail("ParseException expected"); } catch (ParseException e)
   * {
   * 
   * }
