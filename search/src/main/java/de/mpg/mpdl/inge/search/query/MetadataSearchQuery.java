 ...
 * operators to each other. The first criteria should have no logical operator. The next criteria
 * should define a logical operator to connect to the criteria before.<br/>
 * Per default a search query consists of two types of criteria. A list of contentTypes and a list
 * of other search criteria.
 * 
 * @author endres
 * 
 */
public class MetadataSearchQuery extends SearchQuery {
   ...
   * @throws TechnicalException if the object cannot be instantiated
   */
  ...
  /**
   * Creates a query with a list of content types and a list of search criteria.
   * 
   * @param contentTypes list of content types
   * @param criteria list of criteria
   * @throws TechnicalException if the object cannot be instantiated
   */
  public MetadataSearchQuery(ArrayList<String> contentTypes,
      ArrayList<MetadataSearchCriterion> criteria) throws TechnicalException {
    this.contentTypes = new ArrayList<MetadataSearchCriterion>();
    addContentTypeCriterions(contentTypes);
   ...
   * @throws CQLParseException if building up of the tree fails
   * @throws IOException if an io error occurs
   * @throws ParseException if the parsing from the search terms fails
   * @throws TechnicalException if an internal error occurs
   */
  public CQLNode getCqlNode() throws CQLParseException, IOException, ParseException,
      TechnicalException {
  ...
  /**
   * Checks if a set of criteria can be transformed into a cql query. If a query has more than one
   * criteria, the following criteria must have a logical operator, otherwise no valid cql query can
   * be created.
   * 
   ...
   * @throws ParseException if parsing of the searchterms fails
   * @throws TechnicalException if an internal error occurs
   */
  public String getCqlString() throws ParseException, TechnicalException {
    if (!isQueryValid()) {
      throw new TechnicalException(
          "The search query is invalid, missing logical operator at criteria");
    }
   ...
   * {@inheritDoc}
   */
  public String getCqlQuery() throws CQLParseException, IOException, ParseException,
      TechnicalException {
  ...
  /**
   * Generate a three node tree with a criterion and a existing node. The existing node will be
   * attached to a newly created node and a new root node above the two will be returned.
   * 
   * @param node node to be attached to
   * @param criterion criterion for the newly created node
   * @return root node of the two node tree
   * @throws CQLParseException if the tree building fails
   * @throws IOException if an io error occurs
  ...
  private CQLNode generateNodeWithCriterion(CQLNode node, MetadataSearchCriterion criterion)
    ...
    switch (criterion.getLogicalOperator()) {
      case AND:
        newRoot =
            new CQLAndNode(node, criterion.generateCqlTree(), new ModifierSet(
                criterion.getLogicalOperatorAsString()));
        break;
      case OR:
        newRoot =
            new CQLOrNode(node, criterion.generateCqlTree(), new ModifierSet(
                criterion.getLogicalOperatorAsString()));
        break;
      case NOT:
        newRoot =
            new CQLNotNode(node, criterion.generateCqlTree(), new ModifierSet(
                criterion.getLogicalOperatorAsString()));
        break;
      default:
   ...
   * 
   * @param contentT content type criteria
   * @throws TechnicalException if criteria type is not supported
   */
  private void addContentTypeCriterions(ArrayList<String> contentT) throws TechnicalException {
    for (int i = 0; i < contentT.size(); i++) {
      this.contentTypes.add(new MetadataSearchCriterion(
          MetadataSearchCriterion.CriterionType.CONTENT_TYPE, contentT.get(i),
