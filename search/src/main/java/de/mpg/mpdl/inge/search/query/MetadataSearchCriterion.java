 ...
 * metadata search criterion can query multiple indices. This is defined by the criterion type.
 * 
 * @author endres
 * 
 */
public class MetadataSearchCriterion implements Serializable {
  ...
  public enum CriterionType {
    TITLE, ANY, ANY_INCLUDE, ABSTRACT, PERSON, PERSON_ROLE, ORGANIZATION, CREATOR_ORGANIZATION, ORGANIZATION_PIDS, CREATOR_ORGANIZATION_IDS, CREATOR_ORGANIZATION_IDS_WITH_PATH, GENRE, DATE_ANY, DATE_CREATED, DATE_ACCEPTED, DATE_SUBMITTED, DATE_MODIFIED, DATE_PUBLISHED_ONLINE, DATE_ISSUED, DATE_EVENT_START, DATE_EVENT_END, EVENT_INVITATION_STATUS, TOPIC, SOURCE, EVENT, IDENTIFIER, CONTEXT_OBJECTID, CONTEXT_NAME, CREATED_BY_OBJECTID, LANGUAGE, CONTENT_TYPE, OBJECT_TYPE, COMPONENT_ACCESSIBILITY, COMPONENT_STORAGE, COMPONENT_VISIBILITY, COMPONENT_CONTENT_CATEGORY, COMPONENT_COMPOUND_PROPERTIES, LOCAL_TAG, COPYRIGHT_DATE, EMBARGO_DATE, DEGREE, PERSON_IDENTIFIER, LATEST_RELEASE_OBJID, OBJID
  };
  ...
  private static final String INDEX_PERSON =
      "escidoc.publication.creator.person.compound.person-complete-name";
  ...
  private static final String INDEX_CREATOR_ORGANIZATION =
      "escidoc.publication.creator.person.organization.title";
  ...
  private static final String INDEX_CREATOR_ORGANIZATION_IDS =
  ...
  private static final String INDEX_CREATOR_ORGANIZATION_IDS_WITH_PATH =
  ...
  private static final String INDEX_EVENT_INVITATION_STATUS =
      "escidoc.publication.event.invitation-status";
  ...
  private static final String INDEX_COMPONENT_CONTENT_CATEGORY =
      "escidoc.component.content-category";
  ...
  private static final String INDEX_LOCAL_TAG =
      "escidoc.property.content-model-specific.local-tags.local-tag";
  ...
  private static final String INDEX_PERSON_IDENTIFIER =
      "escidoc.publication.creator.person.identifier";
  ...
  /**
   * Constructor with criterion type and empty search term. The search term is empty, because this
   * criterion shall be used to verify if a index is present.
   * 
  ...
  /**
   * Constructor with criterion type, logical operator and empty search term. The search term is
   * empty, because this criterion shall be used to verify if a index is present.
   * 
   * @param type the type of a criteria
   * @param operator logical operator
   * @throws TechnicalException if creation of object fails
   */
  public MetadataSearchCriterion(CriterionType type, LogicalOperator operator)
   ...
   * @param type the type of a criteria
   * @param searchTerm the term to search for
   * @param booleanOperator the boolean operator to use between the index and the search term
   * @throws TechnicalException if creation of object fails
   */
  public MetadataSearchCriterion(CriterionType type, String searchTerm,
      BooleanOperator booleanOperator) throws TechnicalException {
    typeList = new ArrayList<CriterionType>();
    this.searchTerm = searchTerm;
    this.searchIndexes = setIndexByEnum(type);
    this.logicalOperator = LogicalOperator.UNSET;
  ...
  /**
   * Constructor with criterion type, search term, boolean operator and logical operator.
   * 
   * @param type the type of a criteria
   * @param searchTerm the term to search for
   * @param booleanOperator the boolean operator to combine two search criteria
   * @param operator the boolean operator to use between the index and the search
   * @throws TechnicalException if creation of object fails
   */
  public MetadataSearchCriterion(CriterionType type, String searchTerm,
      BooleanOperator booleanOperator, LogicalOperator operator) throws TechnicalException {
    typeList = new ArrayList<CriterionType>();
    this.searchTerm = searchTerm;
    this.searchIndexes = setIndexByEnum(type);
    this.logicalOperator = operator;
   ...
   * @throws TechnicalException if creation of object fails
   */
  public MetadataSearchCriterion(ArrayList<CriterionType> types, String searchTerm)
      throws TechnicalException {
    typeList = new ArrayList<CriterionType>();
    this.searchTerm = searchTerm;
   ...
   * @throws TechnicalException if creation of object fails
   */
  protected MetadataSearchCriterion(ArrayList<CriterionType> types, String searchTerm,
  ...
  private ArrayList<String> setIndexByEnum(CriterionType type) throws TechnicalException {
    ...
    switch (type) {
      ...
      case EVENT_INVITATION_STATUS:
        ...
        break;
      default:
        throw new TechnicalException("The index " + type.name()
            + " is unknown. Cannot map to index name.");
    }
  ...
  public String generateCqlQuery() throws ParseException, TechnicalException {
    QueryParser parser =
        new QueryParser(this.searchTerm, booleanOperatorToString(this.cqlOperator));
  ...
  protected String getCqlQueryFromSubCriteria() throws TechnicalException, ParseException {
    ...
    if (!subCriteria.isEmpty()) {
      ...
      for (int i = 1; i < subCriteria.size(); i++) {
        cqlQuery =
            cqlQuery + " " + subCriteria.get(i).getLogicalOperatorAsString() + " "
  ...
  /**
   * Return the last logical operator as string. If there are sub criteria for this criteria, take
   * the logicoperator from the last subcriteria in the list.
   * 
