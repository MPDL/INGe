...
public class MetadataDateSearchCriterion extends MetadataSearchCriterion {
  ...
  private String composeCqlFragments(String minor, String major) throws ParseException,
      TechnicalException {
  ...
  private String createCqlFragment(String index, String minor, String major) throws ParseException,
      TechnicalException {
    String fromQuery = null;
    String toQuery = null;
    if (minor != null) {
      minor = normalizeFromQuery(minor);
      QueryParser parserFrom =
          new QueryParser(minor, booleanOperatorToString(BooleanOperator.GREATER_THAN_EQUALS));
      parserFrom.addCQLIndex(index);
      fromQuery = parserFrom.parse();
    }
    if (major != null) {
      String[] majorParts = normalizeToQuery(major);
      QueryParser parserTo =
          new QueryParser(majorParts[0], booleanOperatorToString(BooleanOperator.LESS_THAN_EQUALS));
      parserTo.addCQLIndex(index);
      toQuery = parserTo.parse();
      for (int i = 1; i < majorParts.length; i++) {
        QueryParser parserNotTo =
            new QueryParser(majorParts[i], booleanOperatorToString(BooleanOperator.EQUALS));
