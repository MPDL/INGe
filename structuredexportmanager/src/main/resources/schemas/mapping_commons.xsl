...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:misc="http://www.editura.de/ns/2012/misc" xmlns:tools="http://www.editura.de/ns/2012/tools" xmlns:hidden="http://www.editura.de/ns/2012/misc-hide" xmlns:AuthorDecoder="java:de.mpg.mpdl.inge.transformation.util.creators.AuthorDecoder" xmlns:Util="java:de.mpg.escidoc.services.transformation.Util" xmlns:dc="${xsd.metadata.dc}" xmlns:dcterms="${xsd.metadata.dcterms}" xmlns:eprints="http://purl.org/eprint/terms/" xmlns:escidocComponents="${xsd.soap.item.components}" xmlns:escidocItem="${xsd.soap.item.item}" xmlns:escidocItemList="${xsd.soap.item.itemlist}" xmlns:escidocMetadataRecords="${xsd.soap.common.metadatarecords}" xmlns:eterms="${xsd.metadata.escidocprofile.types}" xmlns:event="${xsd.metadata.event}" xmlns:eves="http://purl.org/escidoc/metadata/ves/0.1/" xmlns:file="${xsd.metadata.file}" xmlns:foaf="http://xmlns.com/foaf/0.1/" xmlns:organization="${xsd.metadata.organization}" xmlns:person="${xsd.metadata.person}" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:prop="${xsd.soap.common.prop}" xmlns:publication="${xsd.metadata.publication}" xmlns:source="${xsd.metadata.source}" xmlns:srel="${xsd.soap.common.srel}" xmlns:xlink="http://www.w3.org/1999/xlink" exclude-result-prefixes="xsl xs xd misc tools hidden AuthorDecoder Util" version="2.0">
	...
	<xsl:param name="escidoc.pubman.external.organisation.id" as="xs:string?">${escidoc.pubman.external.organisation.id}</xsl:param>
					...
					<xsl:call-template name="misc:make_organization-organization">
						...
						<xsl:with-param name="dc:identifier" as="element(dc:identifier)?">
							<xsl:if test="$hidden:write-default-organization-id">
								<xsl:sequence select="misc:create_dc-identifier('', $escidoc.pubman.external.organisation.id)"/>
							</xsl:if>
						</xsl:with-param>
					</xsl:call-template>
				</xsl:when>
	...
	<xsl:function name="hidden:is-generic-organization-organization" as="xs:boolean">
		...
		<xsl:choose>
			...
			<xsl:when test="$organization:organization/dc:identifier/normalize-space() eq normalize-space($escidoc.pubman.external.organisation.id)">
				<xsl:sequence select="true()"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:sequence select="false()"/>
