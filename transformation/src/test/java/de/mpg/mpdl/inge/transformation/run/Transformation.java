...
package de.mpg.mpdl.inge.transformation.run;
...
public class Transformation {
  ...
  /**
   * Give the following arguments: * source-filename: where the the source is located. *
   * source-format: the name of the format, e.g. "edoc" * source-type: the mimetype, e.g.
   * "application/xml" * source-encoding: the character encoding scheme, e.g. "UTF-8" *
   * target-filename: where the the result should be stored. * target-format: the name of the
   * format, e.g. "edoc" * target-type: the mimetype, e.g. "application/xml" * target-encoding: the
   * character encoding scheme, e.g. "UTF-8" * transformation-service (optional): not yet
   * implemented, use "escidoc".
   */
  public static void main(String[] args) throws Exception {
    if (args.length == 8) {
      new Transformation(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7],
          "escidoc");
