...
package de.mpg.mpdl.inge.transformation.transformations.commonPublicationFormats.marc;
...
/*
 * transforms MARC21 (binary) to MarcXML initial author: Stefan Krause, Editura GmbH & Co. KG
 * 
...
public class Marc21ToMarcXMLTransformation implements Configurable, Transformation {
  ...
  public byte[] transform(byte[] src, Format srcFormat, Format trgFormat, String service,
      ...
      RuntimeException {
    if (Util.isFormatEqual(srcFormat, MARC21_FORMAT)
        && Util.isFormatEqual(trgFormat, MARCXML_FORMAT)) {
      ...
      // TODO: because MarcXmlWriter does not produce the correct namespace, I've created a nasty
      // workaround. Need to be cleaned up sometimes, Stf, 2013-03-14

      String temp = new String();
      byte[] output = null;
      try {
        temp =
            result.toString(trgEncoding).replaceAll("xmlns=\"http://www.loc.gov/MARC21/slim\"", "")
                .replaceAll("<collection", "<collection xmlns=\"http://www.loc.gov/MARC21/slim\"");
        output = temp.getBytes(trgEncoding);
      } catch (UnsupportedEncodingException e) {
        throw new RuntimeException("Can't encode the result to the requested encoding: "
            + trgEncoding, e);
      }
      ...
      throw new TransformationNotSupportedException("Marc21ToMarcXML can't transform "
