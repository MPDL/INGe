...
public class OutputTransformation {
   ...
   */
  public byte[] transformSnippetToOutput(byte[] src, Format srcFormat, Format trgFormat,
      String service) throws TransformationNotSupportedException, JRException, IOException,
      TechnicalException {
    byte[] output = null;
    ...
    InputStream csj =
        ResourceUtil.getResourceAsStream(path, OutputTransformation.class.getClassLoader());
    JasperReport jr = (JasperReport) JRLoader.loadObject(csj);

    params.put(JRXPathQueryExecuterFactory.PARAMETER_XML_DATA_DOCUMENT, document);

    JasperPrint jasperPrint =
        JasperFillManager.fillReport(jr, params, new JRXmlDataSource(document, jr.getQuery()
            .getText()));
    ...
    switch (OutFormats.valueOf(trgFormat.getName().toLowerCase())) {
      ...
      case txt:
        ...
        break;
      default:
        throw new TransformationNotSupportedException("Transformation to format "
            + trgFormat.getName() + " is not supported");
    }
