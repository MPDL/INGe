...
/**
 * The class implements the comparison of two EventVOs for the criteria which has to be given in the
 * constructor. Note: this comparator imposes orderings that are inconsistent with equals.
 * 
...
public class VersionHistoryEntryVOComparator implements Comparator<VersionHistoryEntryVO> {
  ...
  /**
   * This method compares the dates of two EventVOs using the modificationDate attribute. Both
   * itemVersions must not be null. If no date is given (is null) it is greater than a given date.
   * 
   * @param event1 The first event version.
   * @param event2 The second pubItem version.
   * @return 0 if the item versions are equal, -1 if the first argument is less than the second, 1
   *         otherwise.
   */
