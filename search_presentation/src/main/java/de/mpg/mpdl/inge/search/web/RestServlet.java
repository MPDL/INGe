...
public class RestServlet extends HttpServlet {
    ...
    try {
      ...
      if (!checkVal(cqlQuery)) {
        resp.sendError(HttpServletResponse.SC_BAD_REQUEST,
            "cqlQuery is not defined in the QueryString: " + qs);
        return;
      }
      ...
      exportFormat =
          "Default".equalsIgnoreCase(exportFormat) ? "Default" : "Test"
              .equalsIgnoreCase(exportFormat) ? "Test" : exportFormat.toUpperCase();
      ...
      if (exportFormat.equals("")) {
        ...
        // txt
      } else if (!(CitationStyleManager.isCitationStyle(exportFormat) || StructuredExportService
      ...
      if (StructuredExportService.isStructuredFormat(exportFormat)) {
        ...
        outputFormat =
            exportFormat.equalsIgnoreCase("XML") ? "xml" : exportFormat
                .equalsIgnoreCase("ESCIDOC_XML") ? "escidoc_xml" : exportFormat
                .equalsIgnoreCase("ESCIDOC_XML_V13") ? "escidoc_xml_v13" : exportFormat
                .toUpperCase().startsWith("EDOC_") ? "xml" : exportFormat
                .equalsIgnoreCase("MARCXML") ? "xml" : FileFormatVO.TEXT_NAME;
      } else
      // citation style
      {
        ...
        else if (CitationStyleManager.getMimeType(exportFormat, outputFormat) == null) {
      ...
      ExportSearchQuery query =
          new ExportSearchQuery(cqlQuery, index, exportFormat, outputFormat, cslConeId);
  ...
  /**
   * Mapping of outputFormat to mime-type. TODO: Get the mapping directly from ItemExportingBean
   * 
   * @param resp
   * @param outputFormat
   */
  ...
  /**
   * Mapping of the outputType file to the correct file extension. TODO: Get the mapping directly
   * from ItemExportingBean
   * 
   * @param outputFormat
