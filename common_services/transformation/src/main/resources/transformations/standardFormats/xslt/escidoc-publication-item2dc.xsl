...
<!-- Transformations from eSciDoc PubItem to OAI DC See mapping: http://colab.mpdl.mpg.de/mediawiki/PubMan_Func_Spec_Dublin_Core_Mapping 
	Author: Vlad Makarenko (initial creation) $Author: $ (last changed) $Revision: 
	$ $LastChangedDate: $ -->
<xsl:stylesheet version="1.0"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
	xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:dc="${xsd.metadata.dc}"
	xmlns:dcterms="${xsd.metadata.dcterms}" xmlns:dcmitype="${xsd.metadata.dcmitype}"
	xmlns:pub="${xsd.metadata.publication}" xmlns:eterms="${xsd.metadata.terms}"
	xmlns:source="${xsd.metadata.source}" xmlns:event="${xsd.metadata.event}"
	xmlns:person="${xsd.metadata.person}" xmlns:organization="${xsd.metadata.organization}">
	...
	<xsl:template match="/">
		...
		<xsl:for-each select="//pub:publication">
			...
			<oai_dc:dc xmlns:oai_dc="http://www.openarchives.org/OAI/2.0/oai_dc/"
					...
					<xsl:choose>
						<xsl:when test="@role = 'http://www.loc.gov/loc.terms/relators/AUT'">
							...
							<xsl:if test="person:person/organization:organization!=''">
								<dc:contributor>
									<xsl:apply-templates mode="dc"
										select="person:person/organization:organization" />
								</dc:contributor>
							</xsl:if>
						</xsl:when>
						<xsl:when
							test="@role = 'http://www.loc.gov/loc.terms/relators/SAD' or @role = 'http://www.loc.gov/loc.terms/relators/CTB' or @role = 'http://www.loc.gov/loc.terms/relators/TRC' or @role = 'http://www.loc.gov/loc.terms/relators/TRL' or @role = 'http://www.loc.gov/loc.terms/relators/HNR'">
							<dc:contributor>
								<xsl:apply-templates mode="dc" />
							</dc:contributor>
							<xsl:if test="person:person/organization:organization!=''">
				...
				<xsl:variable name="stype" select="source:source/@type" />
				...
				<xsl:variable name="source">
					...
					<xsl:for-each
						select="source:source[1][$stype = 'http://purl.org/eprint/type/Book' or $stype = 'http://purl.org/escidoc/metadata/ves/publication-types/proceedings' or $stype = 'http://purl.org/escidoc/metadata/ves/publication-types/issue' or $stype = 'http://purl.org/escidoc/metadata/ves/publication-types/other']/eterms:creator">
						<xsl:if test="./person:person!=''">
							<xsl:value-of
								select="concat(' ', ./person:person/eterms:family-name, ', ', ./person:person/eterms:given-name)" />
						</xsl:if>
					...
					<xsl:for-each
						select="source:source[1][@type='journal' or @type = 'series']/eterms:publishing-info">
						<xsl:value-of
							select="concat(' ', ./dc:publisher, ' ', ./eterms:place, ' ', ./eterms:edition)" />
					</xsl:for-each>
				</xsl:variable>
				...
				<xsl:variable name="format">
					...
					<xsl:text> </xsl:text>
					<xsl:value-of select="source:source[1]/eterms:total-number-of-pages" />
				</xsl:variable>
				...
				<xsl:if test="event:event!=''">
					<xsl:variable name="event-dates">
						...
						<xsl:text>&#8212;</xsl:text>
						<xsl:value-of select="event:event/eterms:end-date" />
					</xsl:variable>
					<xsl:variable name="event">
						...
						<xsl:text>, </xsl:text>
						<xsl:value-of select="$event-dates" />
					</xsl:variable>
					<xsl:if test="$event!=''">
						<dc:relation>
