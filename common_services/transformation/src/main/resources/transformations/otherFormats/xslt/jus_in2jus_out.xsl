...
<xsl:stylesheet version="2.0"
	...
	<xsl:choose>
		...
		<xsl:element name="AUTHORS">
			...
			<xsl:attribute name="totalItemsCount" select="count($item-list/escidocItem:item)"/>
			...
			<xsl:for-each select="$coneResult/rdf:RDF/rdf:Description">
				...
				<xsl:element name="AUTOR">
					...
					<xsl:attribute name="authorListCount" select="count($item-list/escidocItem:item/escidocMetadataRecords:md-records/escidocMetadataRecords:md-record[publication:publication/eterms:creator/person:person/dc:identifier = $currentAuthorId 
...
<!--		<xsl:value-of select="$authorString"></xsl:value-of>-->
			<xsl:value-of select="escidocItem:properties/prop:content-model-specific/dcterms:bibliographicCitation" disable-output-escaping="yes"/>
		<xsl:text> (</xsl:text>
		<xsl:value-of select="@objid"></xsl:value-of>
		<xsl:text>)</xsl:text>
			<xsl:text>
