...
public class ReportTransformation {
   ...
   * institutsId for querying all CoNE persons for the given institute or department. The result is
   * a sorted report, where is a section for each person in CoNE of the given institutsId. The
   * publications of each person are sorted regarding the sort order file.
   * 
   * @param String formatFrom
   * @param String formatTo
   * @param String itemXML
   * @param Map<String, String> Configuration for the XSLT, parameter institutsId for CoNE.
   *        institutsId could be single Id or several Ids separated with whitespace.
   * @return String The generated String with appended sort order
   */
  public String reportTransform(String formatFrom, Format formatTo, String itemXML,
      Map<String, String> configuration) throws TransformationNotSupportedException {

    String xsltUri =
        formatFrom.toLowerCase() + "2" + formatTo.getName().toLowerCase() + "_"
            + ("text/html".equals(formatTo.getType()) ? "html" : "indesign") + ".xsl";
    ...
    try {
      ...
      InputStream in = cl.getResourceAsStream(this.REPORT_XSLT_LOCATION + "/" + xsltUri);
      ...
      transformer.setParameter("indesign-namespace",
          configuration.get(PropertyReader.getProperty("escidoc.report.indesign.namespace")));
  ...
  /**
   * Append sort order to the eSciDoc-XML input file. The sort order is read from the configuration
   * file and has XML format. The sort order is appended before the first item-element. This is
   * needed for the key lookup table in XSLT.
   * 
  ...
  private String appendSortOrderToItemXml(String itemXML) {
    ...
    try {
      String reportSortOrderPath =
          PropertyReader.getProperty("escidoc.transformation.report.sortorder.filename");
      inString =
          ResourceUtil.getResourceAsString(reportSortOrderPath,
              ReportTransformation.class.getClassLoader());
    } catch (Exception e1) {
