...
public class RetrieveTest {
  ...
  public void testAllAction() throws Exception {
    ...
    assertTrue("Request to " + serviceUrl + "iso639-1/all?format=options"
        + " did not return 200 Ok status, but status " + method.getStatusCode(),
  ...
  public void testQueryAction() throws Exception {
    ...
    assertTrue("Request to " + serviceUrl + "ddc/query?q=b&format=html"
        + " did not return 200 Ok status, but " + method.getStatusCode(),
        method.getStatusCode() == 200);
  ...
  public void testDetailAction() throws Exception {
    ...
    assertTrue("Request did not return 200 Ok status, but " + method.getStatusCode(),
        method.getStatusCode() == 200);
   ...
   */
  private String getResponseAsString(GetMethod method) throws IOException,
