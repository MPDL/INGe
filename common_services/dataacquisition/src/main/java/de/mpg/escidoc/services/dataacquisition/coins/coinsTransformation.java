...
package de.mpg.escidoc.services.dataacquisition.coins;
        ...
        public String getCOinS(PubItemVO pubitemVO)
        {
            ...
            if(pubitemVO != null)
            {
                if(pubitemVO.getMetadata() != null)
                {
                    ...
                    {
                        rft_atitle = RFT_ATIITLE_PREFIX + this.htmlEscape(pubitemVO.getMetadata().getTitle().getValue());
                    ...
                    if(pubitemVO.getMetadata().getSources() != null)
                    {
                        if(pubitemVO.getMetadata().getSources().size() > 0)
                        {
                            ...
                            {
                                rft_title = RFT_TITLE_PREFIX + this.htmlEscape(pubitemVO.getMetadata().getSources().get(0).getTitle().getValue());
                    ...
                    {
                        rft_aulast = RFT_AULAST_PREFIX + this.htmlEscape(getLastName(pubitemVO));
                    }
                    
                    if(!getFirstName(pubitemVO).equals(""))
                    {
                        rft_aufirst = RFT_AUFIRST_PREFIX + this.htmlEscape(getFirstName(pubitemVO));
                    }
                    
                    if(!getCompleteNames(pubitemVO).equals(""))
                    {
                        rft_au = RFT_AU_PREFIX + this.htmlEscape(getCompleteNames(pubitemVO));
                    }
                    
                    if(!getISSN(pubitemVO).equals(""))
                    {
                        rft_issn = RFT_ISSN_PREFIX + this.htmlEscape(getISSN(pubitemVO));
                    }
                    
                    if(!getISBN(pubitemVO).equals(""))
                    {
                        rft_isbn = RFT_ISBN_PREFIX + this.htmlEscape(getISBN(pubitemVO));
                    }
                    
                    if(!getSourceIdentifier(pubitemVO).equals(""))
                    {
                        rft_id = RFT_ID_PREFIX + this.htmlEscape(getSourceIdentifier(pubitemVO));
                    }
                    
                    if(!getSourceVolume(pubitemVO).equals(""))
                    {
                        rft_volume = RFT_VOLUME_PREFIX + this.htmlEscape(getSourceVolume(pubitemVO));
                    }
                    
                    if(!getSourceIssue(pubitemVO).equals(""))
                    {
                        rft_issue = RFT_ISSUE_PREFIX + this.htmlEscape(getSourceIssue(pubitemVO));
                    }
                    
                    if(!getSourcePages(pubitemVO).equals(""))
                    {
                        rft_pages = RFT_PAGES_PREFIX + this.htmlEscape(getSourcePages(pubitemVO));
                    }
                    
                    if(!getDate(pubitemVO).equals(""))
                    {
                        rft_date = RFT_DATE_PREFIX + this.htmlEscape(getDate(pubitemVO));
                    }
