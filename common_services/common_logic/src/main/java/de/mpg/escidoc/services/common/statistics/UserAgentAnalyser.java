...
public class UserAgentAnalyser
{
  ...
  Here we go with a big piece: The detection of the OS. The code is mostly focused on Windows for two reasons:
     ...
     2. The version numbers are a little screwed as the OS is represented by two names (Win and Windows) and the version numbers for NT by two different representations: XP and NT 5.1 for example.
*/
  ...
  Here is the main part: The detection of the Browser. Some interesting tricks:
     ...
     1. As Opera has the good idea of including a version of IE in its User-agent, it has to be tested before IE.
     2. Gecko stands for the rendering engine of Mozilla. It has several packagings: Camino & Chimera for the Mac, Galeon for *nix and Phoenix, Firebird, and Firefox for the generic versions from mozilla.org. Additionally, Netscape in its versions greater than 6 also uses Gecko.
     3. KHTML stands for the KDE renderer. It is packaged as Konqueror for Linux and Safari for the Mac.
     4. As most of the User-Agents begin with Mozilla/X.0, it is very hazardous to detect browsers that does rely on this version number to actually describe themselves. Among them is Netscape Communicator that hopefully is finally dying.
*/
    ...
    public static String []getBrowser(String userAgent) {
      ...
      if (userAgent.indexOf("MSIE")>-1) {
          ...
          if ((pos=userAgent.indexOf("MSIE 7"))>-1) {
              res = getArray("MSIE","MSIE7","MSIE"+getVersionNumber(userAgent,pos+4));
          } else
          if ((pos=userAgent.indexOf("MSIE 8"))>-1) {
