...
public class GrantList extends CurrentGrants {
  ...
  public static class Factory {
    ...
    /**
     * Retrieves the grants for a given escidoc object and a given role that are not revoked and
     * only for groups.
     * 
     * @param userHandle A user handle for authentication in the coreservice.
     ...
     * @throws Exception If an error occurs in coreservice or during marshalling/unmarshalling.
     */
    public static GrantList retrieveGrantsForObject(String userHandle, String objectId,
        String roleId) throws RuntimeException {
      try {
        ...
        filter.put("query", new String[] {"\"/properties/assigned-on/id\"=" + objectId + " and "
            + "\"/properties/role/id\"=" + roleId + " and "
            + "\"/properties/revocation-date\"=\"\""});


        String grantListXml = uah.retrieveGrants(filter);
        SearchRetrieveResponseVO res =
            new XmlTransformingBean().transformToSearchRetrieveResponseGrant(grantListXml);
     ...
     * @throws Exception If an error occurs in coreservice or during marshalling/unmarshalling.
     */
    private static GrantList retrieveGrants(String userHandle, HashMap<String, String[]> filter)
        throws RuntimeException {
      try {
        UserAccountHandler uah = ServiceLocator.getUserAccountHandler(userHandle);
        String grantListXml = uah.retrieveGrants(filter);
        GrantList currentGrants =
            (GrantList) IntelligentVO.unmarshal(grantListXml, GrantList.class);
