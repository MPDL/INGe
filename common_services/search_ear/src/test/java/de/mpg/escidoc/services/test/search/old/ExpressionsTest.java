...
public class ExpressionsTest {
  ...
  // StringReader("Mr \"OR\" Mrs NOT Teacher but \"Exactly \\\"this stuff\"");
  // QueryParser parser = new QueryParser(reader);
  // parser.addCQLIndex("escidoc.metadata");
  // String query = parser.parse();
  ...
  // {
  // StringReader reader = new
  // StringReader("(((The) (quick) (brown)) ((fox) (jumps)) ((over) (the)) ((lazy) (dog)(.)))");
  // QueryParser parser = new QueryParser(reader);
  // parser.addCQLIndex("escidoc.metadata");
  // String query = parser.parse();
  ...
  // {
  // StringReader reader = new
  // StringReader("(((The)NOT(quick)AND(brown))((fox)DOES(jump))NOT((over)NOT(the))NOT((lazy)AND(dog).))");
  // QueryParser parser = new QueryParser(reader);
  // parser.addCQLIndex("escidoc.metadata");
  // String query = parser.parse();
  ...
  // * e. g.
  // "(((The)NOT(quick)AND NOT(brown))((fox)DOES(jump))NOT((over)NOT(the))NOT((lazy)AND(dog).))"
  // */
  // @Test
  // public void manyBracketsAndOperatorsNOTallowed()
  // {
  // StringReader reader = new
  // StringReader("(((The)NOT(quick)AND NOT(brown))((fox)DOES(jump))NOT((over)NOT(the))NOT((lazy)AND(dog).))");
  // QueryParser parser = new QueryParser(reader);
  // parser.addCQLIndex("escidoc.metadata");
  // try
  // {
  ...
  // * e. g.
  // "(((The)\"NOT(quick)AND(brown))\"((fox)DOES(jump))NOT((over)NOT(the))NOT((lazy)AND(dog).))"
  // */
  // @Test
