...
-->
<!-- 
	Shared functions/tempates for escidoc2edoc_export.xsl and escidoc2edoc_import.xsl 
	Author: Vlad Makarenko (initial creation) 
	$Author: vmakarenko $ (last changed)
	$Revision: 3737 $ 
	$LastChangedDate: 2010-11-30 19:28:24 +0100 (Tue, 30 Nov 2010) $
-->
	...
	<xsl:template name="creator">
		...
		<xsl:for-each select="*">
			...
			<xsl:element name="creator">
				...
				<xsl:attribute name="{if ($type='import') then 'creatortype' else 'creatorType'}" select="
		...
		<xsl:if test="$OUs">
			...
			<xsl:variable name="da">
				...
				<xsl:variable name="int_affs" select="
					func:getMpgAffiliations(eterms:creator/person:person/organization:organization, $type)
				"/> 
