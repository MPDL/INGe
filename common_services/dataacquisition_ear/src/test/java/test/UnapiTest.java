...
public class UnapiTest {
  ...
  public void unapiTestAllSources() throws Exception {
    ...
    // bmc
    /*
     * url = location + "?id=" + this.bmcId + "&format=bmc"; getMethod = new GetMethod(url); code =
     * ProxyHelper.executeMethod(client, getMethod); this.logger.info("Fetch: " + url +
     * "    Response: " + code); Assert.assertEquals(200, code);
     */
  ...
  public void unapiTestOneSourceFull() throws Exception {
    ...
    for (String format : formatsList) {
      // Skipping marc formats. Implementation of MarcTransformation conflicts with the
      // configuration in the explain-transformations.xml.
