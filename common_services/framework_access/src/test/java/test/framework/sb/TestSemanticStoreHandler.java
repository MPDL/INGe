...
public class TestSemanticStoreHandler extends TestItemBase {
  ...
  public void findTriplesOfObject() throws Exception {
    ...
    String param =
        "<param>" + "<query>&lt;info:fedora/" + id + "&gt; * *</query>" + "<format>"
            + OUTPUT_FORMAT + "</format>" + "</param>";
  ...
  public void findTriplesOfIllegalObject() throws Exception {
    String param =
        "<param>" + "<query>&lt;info:fedora/" + ILLEGAL_ID + "&gt; * *</query>" + "<format>"
            + OUTPUT_FORMAT + "</format>" + "</param>";
    logger.debug("Param=" + param);
    long zeit = -System.currentTimeMillis();
    try {
  ...
  public void findObjectIsRevisionOf() throws Exception {
    String param =
        "<param>"
            + "<query>&lt;info:fedora/"
            + createRelatedItems()
            + "&gt; http://www.escidoc.de/ontologies/mpdl-ontologies/content-relations#isRevisionOf *</query>"
  ...
  public void findRevisionsOfObject() throws Exception {
    String param =
        "<param>" + "<query>* " + PREDICATE_ISREVISIONOF + " &lt;info:fedora/"
            + createRelatedItems() + "&gt;</query>" + "<format>" + OUTPUT_FORMAT + "</format>"
            + "</param>";
    logger.debug("Param=" + param);
    long zeit = -System.currentTimeMillis();
    try {
  ...
  private String createRelatedItems() throws Exception {
    ...
    String param =
        "<param last-modification-date=\"" + sourceMd + "\">" + "    <relation>"
            + "        <targetId>" + targetId + "</targetId>" + "        <predicate>"
            + PREDICATE_ISREVISIONOF + "</predicate>" + "    </relation>" + "</param>";
