...
public class CreateOrganizationsTest extends TestBase {
  ...
  /**
   * This Map contains a mapping between abbreviations and object ids. See the
  ...
  public static void setUpBeforeClass() throws Exception {
    // create the affiliation structure (as the structure is only read in the test methods, this
    // can/should be done
    // here)
    // String systemAdministratorUserHandle = loginSystemAdministrator();
    // CreateOrganizationsTest.abbreviationToObjIdMapping =
  ...
  public void testCreateOrganizationListFromAffiliationHG() throws Exception {
    ...
    String systemAdministratorUserHandle = loginSystemAdministrator();
    logger.debug(methodName + "() - String systemAdministratorUserHandle="
        + systemAdministratorUserHandle);

    // retrieve affiliation 'HG' from the framework
    String hg2XML =
        ServiceLocator.getOrganizationalUnitHandler(systemAdministratorUserHandle).retrieve(
            abbreviationToObjIdMapping.get("objectIdHG"));
    // logger.debug("XML "+hg2XML);
    AffiliationVO hg =
        ((XmlTransforming) getService("ejb:common_logic_ear/common_logic/XmlTransformingBean!"
            + XmlTransforming.class.getName())).transformToAffiliation(hg2XML);
    // get the organization list for HG:
    orgList =
        ((DataGathering) getService("ejb:common_logic_ear/common_logic/DataGatheringBean!"
            + DataGathering.class.getName())).createOrganizationListFromAffiliation(
            systemAdministratorUserHandle, hg);
  ...
  public void testCreateOrganizationListFromAffiliationZEL2() throws Exception {
    ...
    String zel2XML =
        ServiceLocator.getOrganizationalUnitHandler(systemAdministratorUserHandle).retrieve(
            abbreviationToObjIdMapping.get("objectIdZEL2"));
    logger.debug(methodName + "() - String zel2XML=\n"
        + toString(getDocument(zel2XML, false), false));
    AffiliationVO zel2 =
        ((XmlTransforming) getService("ejb:common_logic_ear/common_logic/XmlTransformingBean!"
            + XmlTransforming.class.getName())).transformToAffiliation(zel2XML);
    // get the organization list for ZEL2:
    orgList =
        ((DataGathering) getService("ejb:common_logic_ear/common_logic/DataGatheringBean!"
            + DataGathering.class.getName())).createOrganizationListFromAffiliation(
            systemAdministratorUserHandle, zel2);
    ...
    assertTrue(affPathNames[0]
        .startsWith("Zentrale Einrichtung Lichtmikroskopie des MPI für Medizinische Forschung_2"));
  ...
  public void testCreateOrganizationListFromAffiliationFuBerlin() throws Exception {
    ...
    String fuBerlinXML =
        ServiceLocator.getOrganizationalUnitHandler(systemAdministratorUserHandle).retrieve(
            abbreviationToObjIdMapping.get("objectIdFuBerlin"));
    logger.debug(methodName + "() - String fuBerlinXML=\n"
        + toString(getDocument(fuBerlinXML, false), false));
    AffiliationVO fuBerlin =
        ((XmlTransforming) getService("ejb:common_logic_ear/common_logic/XmlTransformingBean!"
            + XmlTransforming.class.getName())).transformToAffiliation(fuBerlinXML);
    // get the organization list for FU-BERLIN:
    orgList =
        ((DataGathering) getService("ejb:common_logic_ear/common_logic/DataGatheringBean!"
            + DataGathering.class.getName())).createOrganizationListFromAffiliation(
            systemAdministratorUserHandle, fuBerlin);
    ...
    for (OrganizationVO org : orgList) {
      ...
      assertTrue("Unerwartet: affPathNames[0]:" + affPathNames[0],
          affPathNames[0].startsWith("Freie Universität Berlin"));
      ...
      if (affPathNames[1]
          .startsWith("Fraunhofer-Gesellschaft zur Förderung der angewandten Forschung e.V.")) {
        fgOccured = true;
