...
public class FindRevisionsOfItemTest extends TestBase {
  ...
  private static DataGathering getDataGathering() throws NamingException {
    return (DataGathering) getService("ejb:common_logic_ear/common_logic/DataGatheringBean!"
        + DataGathering.class.getName());
  }
  ...
  public void findRevisionsOfItem() throws Exception {
    ...
    PubItemVO pubItem =
        ((XmlTransforming) getService("ejb:common_logic_ear/common_logic/XmlTransformingBean!"
            + XmlTransforming.class.getName())).transformToPubItem(item);
    logger.debug("Target is " + pubItem.getVersion().getObjectId());

    // Create two revisions
    // revision 1
    String source =
        ServiceLocator.getItemHandler(userHandle).create(readFile(ITEM_WITHOUT_COMPONENTS_FILE));
    String sourceId1 = getObjid(source);
    String sourceMd = getLastModificationDate(source);
    // add relation source 1 -- isRevisionOf --> target
    String param =
        "<param last-modification-date=\"" + sourceMd + "\">" + "    <relation>"
            + "        <targetId>" + pubItem.getVersion().getObjectId() + "</targetId>"
            + "        <predicate>" + PREDICATE_ISREVISIONOF + "</predicate>" + "    </relation>"
            + "</param>";
    ...
    List<RelationVO> revisions =
        getDataGathering().findRevisionsOfItem(userHandle, pubItem.getVersion());
    // Check the result
    assertNotNull(revisions);
    assertTrue(revisions.size() == 2);
    assertTrue(revisions.get(0).getTargetItemRef().getObjectId()
        .equals(pubItem.getVersion().getObjectId()));
    // assertTrue(revisions.get(0).getSourceItemRef().getObjectId().equals(sourceId1));
    assertTrue(revisions.get(1).getTargetItemRef().getObjectId()
        .equals(pubItem.getVersion().getObjectId()));
