 ...
 */
public class Bug282InconsistentBehaviourOfMethodRetrievePathListTest extends
  ...
  /**
   * Checks if FIZ bugzilla issue #213 still exists (When you add components to an item and update
   * it, the framework sets the component-type of the added components to "null".) The test directly
   * accessed the framework (i.e. does not use the intermediate services in the logic layer.)
   * 
   * @throws Exception
   */
  @Test
  public void testBug282InconsistentBehaviourOfMethodRetrievePathList() throws Exception {
    ...
    String objectIdMPG =
        AffiliationCreator.createTopLevelAffiliation(affiliationVOMPG,
            systemAdministratorUserHandle, uniquer);
    ...
    String objectIdMPIFG =
        AffiliationCreator.createSubAffiliation(affiliationVOMPIFG, parentObjIds,
            systemAdministratorUserHandle, uniquer);
    ...
    List<OrganizationVO> organizationListMPIFG =
        createOrganizationListFromAffiliation(librarianUserHandle, affiliationVOMPIFG);
    ...
    if (affiliationVOMPIFG.getMetadataSets().size() > 0
        && affiliationVOMPIFG.getMetadataSets().get(0) instanceof MdsOrganizationalUnitDetailsVO
        && affiliationVOMPG.getMetadataSets().size() > 0
        && affiliationVOMPG.getMetadataSets().get(0) instanceof MdsOrganizationalUnitDetailsVO) {
      MdsOrganizationalUnitDetailsVO detailsVOMPIFG =
          (MdsOrganizationalUnitDetailsVO) affiliationVOMPIFG.getMetadataSets().get(0);
      MdsOrganizationalUnitDetailsVO detailsVOMPG =
          (MdsOrganizationalUnitDetailsVO) affiliationVOMPG.getMetadataSets().get(0);
      assertEquals(mpifgAffiliationPath, detailsVOMPIFG.getName() + ORGANIZATION_NAME_SEPARATOR
          + detailsVOMPG.getName());
    } else {
      fail("Urganizaional unit details not found");
    }

    // createOrganizationListFromAffiliation for MPG
    List<OrganizationVO> organizationListMPG =
        createOrganizationListFromAffiliation(librarianUserHandle, affiliationVOMPG);
    ...
    if (affiliationVOMPG.getMetadataSets().size() > 0
        && affiliationVOMPG.getMetadataSets().get(0) instanceof MdsOrganizationalUnitDetailsVO) {
      MdsOrganizationalUnitDetailsVO detailsVOMPG =
          (MdsOrganizationalUnitDetailsVO) affiliationVOMPG.getMetadataSets().get(0);
