 ...
 * @version $Revision$ $LastChangedDate: 2007-11-07 12:04:29 +0100 (Mi, 07 Nov 2007) $
 * @revised by MuJ: 20.09.2007
 */
public class TransformPubItemVersionListIntegrationTest extends XmlTransformingTestBase {
  /**
  ...
  public static void setUpBeforeClass() throws Exception {
    ...
    itemSorting =
        (ItemSorting) getService("ejb:common_logic_ear/common_logic/ItemSortingBean!"
            + ItemSorting.class.getName());
  }
  ...
  private PubItemVO createItem(String userHandle) throws TechnicalException, ServiceException,
      RemoteException, URISyntaxException {
  ...
  private PubItemVO updateItem(PubItemVO itemVO, String userHandle) throws TechnicalException,
    ...
    PubItemVO updatedItemVO = xmlTransforming.transformToPubItem(updatedItemXml);
  ...
  public void testTransformPubItemVersionList() throws Exception {
    ...
    logger.info("transformToVersionHistoryEntryVOList(" + itemId + ") -> " + zeit + "ms");
    ...
    for (int i = 0; i < updateCount + 1; i++) {
      ...
      logger.debug(" -reference.versionNumber: " + ref.getVersionNumber());
      Date modDate = pubItemVersion.getModificationDate();
      assertNotNull(modDate);
      logger.debug(" -modificationDate: "
          + new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ").format(modDate));
    ...
    List<VersionHistoryEntryVO> sortedVersionList =
        itemSorting.sortItemVersionList(versionList, VersionHistoryEntryVOComparator.Criteria.DATE,
