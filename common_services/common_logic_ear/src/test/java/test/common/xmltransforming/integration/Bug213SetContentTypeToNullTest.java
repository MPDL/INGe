...
public class Bug213SetContentTypeToNullTest extends XmlTransformingTestBase {
  ...
  public void testBug213SetContentTypeToNull() throws Exception {
    ...
    String testFileName =
        "test/xmltransforming/integration/bug213SetContentTypeToNullTest/Bundhose_PLU-213.jpg";
    ...
    mdsFileVO.setSize((int) ResourceUtil.getResourceAsFile(testFileName,
        Bug213SetContentTypeToNullTest.class.getClassLoader()).length());
    ...
    logger.info("PubItemVO with one file transformed to item(XML) for create."
        + "Content type of file: " + pubItemVOPreCreate.getFiles().get(0).getContentCategory());
    ...
    Node componentType =
        selectSingleNode(itemDoc, "/escidocItem:item/escidocComponents:components");// /escidocComponents:component/escidocComponents:properties/escidocComponents:component-type");
    ...
    fileVO = new FileVO();
    testFileName =
        "test/xmltransforming/integration/bug213SetContentTypeToNullTest/Koax-System PCE 213.jpg";
    // first upload the file to the framework
    fileVO.setContent(uploadFile(testFileName, "image/jpeg", scientistUserHandle).toString());
    // set some properties of the FileVO (mandatory fields first of all)
    fileVO
    ...
    // logger.info("PubItemVO with added second file transformed to item(XML) for update." +
    ...
    String pubItemXMLPostUpdate =
        ServiceLocator.getItemHandler(user.getHandle()).update(id, pubItemXMLPreUpdate);
    ...
    Node componentType1 =
        selectSingleNode(
            itemDoc,
            "/escidocItem:item/escidocComponents:components/escidocComponents:component/escidocComponents:properties/escidocComponents:component-type");
