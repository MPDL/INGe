...
public class ItemUpdateAndBaseURLTest extends XmlTransformingTestBase {
  ...
  /**
   * Checks whether the attribute item.@xml:base that is given back by the framework contains the
   * correct framework URL.
   * 
   * @throws Exception
   */
  @Test
  public void testItemUpdateViaStagingServletXmlBase() throws Exception {
    ...
    logger.info("File uploaded to staging area. Retrieved URL stored in FileVO.content[String]: "
        + stagingURL);
    ...
    mdsFileVO.setSize((int) ResourceUtil.getResourceAsFile(testFileName,
        ItemUpdateAndBaseURLTest.class.getClassLoader()).length());
