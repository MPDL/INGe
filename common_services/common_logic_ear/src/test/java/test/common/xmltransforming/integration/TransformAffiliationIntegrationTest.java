...
public class TransformAffiliationIntegrationTest extends TestBase {
  ...
  /**
   * This Map contains a mapping between abbrevations and object ids. See the
   * {@link test.common.AffiliationCreator} class for details.
   */
  ...
  /**
   * Test method for
   * {@link de.mpg.escidoc.services.common.XmlTransforming#transformToOrganizationalUnit(java.lang.String)}
   * in interaction with the framework.
   * 
   * @throws Exception
   */
  @Test
  public final void testTransformToOrganizationalUnitCreate() throws Exception {
    ...
    logger
        .debug("testTransformToOrganizationalUnitCreate() - String systemAdministratorUserHandle="
    ...
    if (affiliationVOPreCreate.getMetadataSets().size() > 0
        && affiliationVOPreCreate.getMetadataSets().get(0) instanceof MdsOrganizationalUnitDetailsVO) {
      MdsOrganizationalUnitDetailsVO detailsVO =
          (MdsOrganizationalUnitDetailsVO) affiliationVOPreCreate.getMetadataSets().get(0);
    ...
    String organizationalUnitPreCreate =
        xmlTransforming.transformToOrganizationalUnit(affiliationVOPreCreate);
    ...
    String organizationalUnitPostCreate =
        ServiceLocator.getOrganizationalUnitHandler(systemAdministratorUserHandle).create(
    ...
    zeit = -System.currentTimeMillis();
    AffiliationVO affiliationVOPostCreate =
        xmlTransforming.transformToAffiliation(organizationalUnitPostCreate);
    zeit += System.currentTimeMillis();
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug("affiliationVOPreCreate.creationDate: "
          + affiliationVOPreCreate.getCreationDate());
      logger.debug("affiliationVOPostCreate.creationDate: "
          + affiliationVOPostCreate.getCreationDate());
    ...
    ServiceLocator.getOrganizationalUnitHandler(systemAdministratorUserHandle).delete(
        affiliationVOPostCreate.getReference().getObjectId());
  ...
  public final void testTransformToOrganizationalUnitUpdate() throws Exception {
    ...
    if (affiliationVOPreUpdate.getMetadataSets().size() > 0
        && affiliationVOPreUpdate.getMetadataSets().get(0) instanceof MdsOrganizationalUnitDetailsVO) {
      MdsOrganizationalUnitDetailsVO detailsVO =
          (MdsOrganizationalUnitDetailsVO) affiliationVOPreUpdate.getMetadataSets().get(0);
      detailsVO.setCity("Freising");

      detailsVO.getIdentifiers().add(
          new IdentifierVO(IdentifierVO.IdType.URI, "http://www.mpg.de/updated"));
    ...
    zeit = -System.currentTimeMillis();
    String organizationalUnitPreUpdate =
        xmlTransforming.transformToOrganizationalUnit(affiliationVOPreUpdate);
    zeit += System.currentTimeMillis();
    logger.info("transformToOrganizationalUnit() ->" + zeit + "ms\n"
    ...
    String organizationalUnitPostUpdate =
        ServiceLocator.getOrganizationalUnitHandler(systemAdministratorUserHandle).update(
            affiliationObjectIdMPG, organizationalUnitPreUpdate);
  ...
  /**
   * This test creates and retrieves three top-level affiliations (Max Planck Society (MPG),
   * Helmholtz Association and Fraunhofer Society) and some sub-affiliations. Along the way it tests
   * if the child-parent-relationships of the created affiliations in the frameworks are correct.
   * The created structure is as follows:
   * MPG->(MPIFG,MPI-G,FML,MPIMF->(ZEL,MPH-HD->(ZEL_2->(FU-BERLIN_2))),FU-BERLIN // HG->FU-BERLIN //
   * FG->FU-BERLIN
   * 
   * @throws Exception
   */
  ...
  public final void testRetrieveAffiliationStructure() throws Exception {
    ...
    String systemAdministratorUserHandle = loginSystemAdministrator();
    logger
        .debug("testCreateAndRetrieveAffiliationStructure() - String systemAdministratorUserHandle="
            + systemAdministratorUserHandle);

    // create a new, distinct affiliation structure
    TransformAffiliationIntegrationTest.abbreviationToObjIdMapping =
        AffiliationCreator.createAffiliationStructure(systemAdministratorUserHandle);
    ...
    String mpgChilds =
        ServiceLocator.getOrganizationalUnitHandler(systemAdministratorUserHandle)
            .retrieveChildObjects(abbreviationToObjIdMapping.get("objectIdMPG"));
    ...
    String mpimfChilds =
        ServiceLocator.getOrganizationalUnitHandler(systemAdministratorUserHandle)
            .retrieveChildObjects(abbreviationToObjIdMapping.get("objectIdMPIMF"));
    ...
    String mphhdChilds =
        ServiceLocator.getOrganizationalUnitHandler(systemAdministratorUserHandle)
            .retrieveChildObjects(abbreviationToObjIdMapping.get("objectIdMPH_HD"));
    ...
    String zel2Childs =
        ServiceLocator.getOrganizationalUnitHandler(systemAdministratorUserHandle)
            .retrieveChildObjects(abbreviationToObjIdMapping.get("objectIdZEL2"));
    ...
    String topLevelAffiliations =
        ServiceLocator.getOrganizationalUnitHandler(systemAdministratorUserHandle)
            .retrieveOrganizationalUnits(filter.toMap()); // TODO
    ...
    List<AffiliationVO> topLevelAffiliationList =
        xmlTransforming.transformToAffiliationList(topLevelAffiliations);
    int counter = 0;
    ...
    for (AffiliationVO affiliation : topLevelAffiliationList) {
      ...
      if (affiliationObjid.equals(abbreviationToObjIdMapping.get("objectIdMPG"))) {
        counter++;
        ObjectComparator oc =
            new ObjectComparator(affiliation, AffiliationCreator.getTopLevelAffiliationMPG());
        List<String> difflist = oc.getDiffs();
        if (logger.isDebugEnabled()) {
          StringBuffer sb = new StringBuffer();
      ...
      if ((affiliationObjid.equals(abbreviationToObjIdMapping.get("objectIdFG")))
          || (affiliationObjid.equals(abbreviationToObjIdMapping.get("objectIdHG")))) {
        counter++;
      }
    }
    ...
    List<AffiliationVO> mpgChildAffiliationList =
        xmlTransforming.transformToAffiliationList(mpgChildAffiliations);
    if (logger.isDebugEnabled()) {
      ...
      for (AffiliationVO childAffiliation : mpgChildAffiliationList) {
        if (childAffiliation.getMetadataSets().size() > 0
            && childAffiliation.getMetadataSets().get(0) instanceof MdsOrganizationalUnitDetailsVO) {
          MdsOrganizationalUnitDetailsVO detailsVO =
              (MdsOrganizationalUnitDetailsVO) childAffiliation.getMetadataSets().get(0);
    ...
    counter = 0;
    for (AffiliationVO childAffiliation : mpgChildAffiliationList) {
      if (childAffiliation.getReference().getObjectId()
          .equals(abbreviationToObjIdMapping.get("objectIdMPIMF"))) {
        counter++;
        ObjectComparator oc =
            new ObjectComparator(AffiliationCreator.getAffiliationMPIMF(), childAffiliation);
        List<String> difflist = oc.getDiffs();
        if (logger.isDebugEnabled()) {
          StringBuffer sb = new StringBuffer();
        ...
        // There are 7 fields that must have changed:
        // 7: name, parentAffiliations, reference, creationDate, lastModificationDate, creator and
        // status
        assertEquals(7, difflist.size());
        ...
        assertEquals(childAffiliation.getParentAffiliations().get(0).getObjectId(),
            abbreviationToObjIdMapping.get("objectIdMPG"));
      }
    }
    assertEquals(1, counter);

    // get childs of MPIMF
    String mpimfChildAffiliations =
        ServiceLocator.getOrganizationalUnitHandler(systemAdministratorUserHandle)
            .retrieveChildObjects(abbreviationToObjIdMapping.get("objectIdMPIMF"));
    logger.info("child affiliations of MPIMF retrieved.");
    List<AffiliationVO> mpimfChildAffiliationList =
        xmlTransforming.transformToAffiliationList(mpimfChildAffiliations);
    // check number of children
    ...
    String specificAffiliations =
        ServiceLocator.getOrganizationalUnitHandler(systemAdministratorUserHandle)
            .retrieveOrganizationalUnits(filter.toMap()); // TODO
    logger.info("specific affiliations retrieved.");
    List<AffiliationVO> specificAffiliationList =
        xmlTransforming.transformToAffiliationList(specificAffiliations);
    if (logger.isDebugEnabled()) {
    ...
    // check number of children
    assertEquals("Not all of the specific affiliations could be found!", 4,
        specificAffiliationList.size());
