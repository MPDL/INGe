...
public class TransformPubItemResultListIntegrationTest extends XmlTransformingTestBase {
  ...
  private PubItemVO createAndReleaseItem(String userHandle, String itemTitle)
    ...
    logger.info("PubItem '" + createdItemId + "' created. Title: "
        + createdItemVO.getMetadata().getTitle());
  ...
  private ContainerVO createAndReleaseContainer(String userHandle, String containerTitle)
    ...
    logger.info("Container '" + createdContainerId + "' created. Title: "
  ...
  public void testTransformPubItemResultListToItemList() throws Exception {
    ...
    for (String itemRef : itemRefs) {
      ...
      ihr.withdraw(itemRef, "<param last-modification-date=\"" + md
          + "\"><withdraw-comment>The item is withdrawn." + "</withdraw-comment></param>");
  ...
  /**
   * Creates released a container, then searches for it and transforms the result. Afterwards, the
  ...
  public void testTransformContainerResultToContainerVO() throws Exception {
    ...
    // tendres: the search for the created container is disabled since it's not reliable, sometimes
    // it works,
    ...
    // SearchResultElement containerResult =
    // xmlTransforming.transformToSearchResult(searchResultContainer);
    ...
    //
    // MdsFacesContainerVO actual = (MdsFacesContainerVO)
    // containerResultVO.getMetadataSets().get(0);
    //
    ...
    chr.withdraw(containerRef, "<param last-modification-date=\"" + md
        + "\"><withdraw-comment>The container is withdrawn." + "</withdraw-comment></param>");
  ...
  public void testTransformAffiliationResultToAffiliationVO() throws Exception {
    final String affiliationTitle =
        "testTransformAffiliationResultToAffiliationVO" + (new Date().getTime()) + "";

    // create an ou
    AffiliationVO affiliation = createAndOpenAffiliation(adminUserHandle, affiliationTitle);
    MdsOrganizationalUnitDetailsVO expected =
    ...
    // tendres: the search for the created affiliation is disabled since it's not reliable,
    // sometimes it works,
    // sometimes not. in debug it works, in run mode it doesn't etc. etc.
    ...
    // SearchResultElement affiliationResult =
    // xmlTransforming.transformToSearchResult(searchResultAffiliation);
    ...
    //
    // assertTrue(affiliationResultVO.getMetadataSets().get(0) instanceof
    // MdsOrganizationalUnitDetailsVO);
    //
    // MdsOrganizationalUnitDetailsVO actual = (MdsOrganizationalUnitDetailsVO)
    // affiliationResultVO.getMetadataSets().get(0);
    //
    // assertEquals(expected.getName(), actual.getName());
    ...
    ouhr.close(affiliationRef, "<param last-modification-date=\"" + md + "\"></param>");
  ...
  private AffiliationVO createAndOpenAffiliation(String adminUserHandle2, String affiliationTitle)
      throws Exception {
    ...
    AffiliationVO createdAffiliationVO =
        xmlTransforming.transformToAffiliation(createdAffiliationXml);
    ...
    logger.info("OU '" + createdAffiliationId + "' created. Title: "
  ...
  private SearchRetrieveResponseType search(String query, String index) throws TechnicalException {
    ...
    try {
      searchResult =
          ServiceLocator.getSearchHandler(index).searchRetrieveOperation(searchRetrieveRequest);
    } catch (Exception e) {
      throw new TechnicalException(e);
    }
