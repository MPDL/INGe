...
/**
 * Utility class to remove all XSD annotations from all .xsd files in a certain folder structure.
 * Although XSD annotations/documentation is supposed to clear things up, they sometimes cause the
 * opposite: The overview on the structure of an XSD gets lost. This class helps against this
 * undesired effect.
 * 
    ...
    for (File inFile : xsds) {
      try {
        ...
        while ((firstAnnotationBeginPos = xml.indexOf(annotationBegin)) != -1) {
          ...
          xml =
              xml.substring(0, previousElementEnd + 1)
                  + xml.substring(firstAnnotationEndPos + annotationEnd.length(), xml.length());

        }

        String subfolder = inFile.getParent().substring(XSD_INPUT_FOLDER.length());
        String outFileName =
            XSD_OUTPUT_FOLDER + subfolder + System.getProperty("file.separator") + inFile.getName();
