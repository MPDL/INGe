...
public class TestHelper {
  ...
  public static XmlTransforming getXmlTransforming() throws NamingException {
    ...
    XmlTransforming xmlTransforming =
        (XmlTransforming) context.lookup(XmlTransforming.SERVICE_NAME);
  ...
  public static ValidationTransforming getValidationTransforming() throws NamingException {
    InitialContext context = new InitialContext();
    ValidationTransforming vTransforming =
        (ValidationTransforming) context
   ...
   * 
   * @param comment A string denoting the currently executed action.
   * @param warnTime A time span in milliseconds. If the action took longer than this, a warning
   ...
   * @param warnTime A time span in milliseconds. If the action took longer than this, a warning
   *        message is displayed.
   * @param maxTime A time span in milliseconds. If the action took longer than this, the test will
   *        fail.
   */
  public static void logTime(final String comment, final long warnTime, final long maxTime) {
    ...
    if (maxTime != -1 && duration > maxTime) {
      throw new RuntimeException("Maximal execution time exceeded (max. time was " + maxTime
          + ", exec. took " + duration + ")");
    }
    if (warnTime != -1 && duration > warnTime) {
      LOGGER.warn("Expected execution time exceeded (exp. time was " + warnTime + ", exec. took "
          + duration + ")");
    }
