...
public class BmcProcessor extends GenericXmlProcessor
	{
		...
		protected void addItems(Node root)
			{
				if (isBmcArticle(root))
					...
					// I've never seen a collection of BMC records, but a generic outermost element will do the job, Stf, 2013-03-22
					{
		...
		private Boolean isBmcArticle(Node node)
			{
				if (node == null || node.getLocalName() == null) // to prevent a NPE
					{
						return false;
					}
