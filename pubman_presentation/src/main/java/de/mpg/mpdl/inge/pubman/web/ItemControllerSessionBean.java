...
public class ItemControllerSessionBean extends FacesBean {
   ...
   * @return string, identifying the page that should be navigated to after this methodcall
   */
  public String withdrawCurrentPubItem(String navigationRuleWhenSuccessfull,
   ...
   * @return string, identifying the page that should be navigated to after this methodcall
   */
  public String createNewRevision(String navigationRuleWhenSuccessfull, ContextRO pubContextRO,
      PubItemVO pubItem, String revisionDescription) {
    if (logger.isDebugEnabled()) {
      logger.debug("Creating a new Revision of item with ID: " + pubItem.getVersion().getObjectId()
          + " for context with ID: " + pubContextRO.getObjectId() + " with revision description: "
          + revisionDescription);
    }
   ...
   */
  private ItemRO submitPubItem(PubItemVO pubItem, String comment) throws PubItemLockedException,
      PubItemStatusInvalidException, PubItemNotFoundException, SecurityException,
      TechnicalException {
    ...
    if (getCurrentWorkflow().equals(PubItemDepositing.WORKFLOW_SIMPLE)) {
      item =
          this.pubItemPublishing.releasePubItem(item.getVersion(), item.getModificationDate(),
              comment, loginHelper.getAccountUser());
    }
   ...
   * @throws Exception if framework access fails
   */
  private ItemRO acceptPubItem(PubItemVO pubItem, String comment) throws PubItemLockedException,
      PubItemStatusInvalidException, PubItemNotFoundException, SecurityException,
      TechnicalException {
