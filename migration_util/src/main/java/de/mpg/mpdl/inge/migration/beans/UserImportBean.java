...
public class UserImportBean {
	...
	public void importSingleUser(String id) throws Exception {
		...
		uri = new URIBuilder(escidocUrl + userPath + "/" + id + "/resources/current-grants")
				.addParameter("maximumRecords", String.valueOf(500)).addParameter("startRecord", String.valueOf(1))
				.build();
		...
		uri = new URIBuilder(escidocUrl + userPath + "/" + id + "/resources/attributes")
				.addParameter("maximumRecords", String.valueOf(500)).addParameter("startRecord", String.valueOf(1))
				.build();
		...
		log.info("Saving user" + accountUser.getName() + " - "
	...
	private AccountUserDbVO transformToNew(AccountUserVO oldAccountUserVO, List<GrantVO> grants,
		...
		*/
		...
		if (grants != null) {
			...
			usr_group_admin = grants
					.removeIf((GrantVO vo) -> vo.getRole().contains("user-group-administrator"));
			yb_dep = grants.removeIf((GrantVO vo) -> vo.getRole().contains("depositor")
					&& vo.getObjectRef().equals("/ir/context/escidoc:1600138"));
