...
public class MigrationTests {
	...
	private void importContexts() throws Exception {
		URI uri = new URIBuilder("https://qa-coreservice.mpdl.mpg.de/ir/contexts")
				.addParameter("maximumRecords", "5000").build();
		...
		try {
			SearchRetrieveResponseVO<de.mpg.mpdl.inge.model.valueobjects.ContextVO> contextList = XmlTransformingService
					.transformToSearchRetrieveResponse(contextXml);
	...
	private void importAffs() throws Exception {
		...
		try {
			SearchRetrieveResponseVO<AffiliationVO> ouList = XmlTransformingService
					.transformToSearchRetrieveResponseOrganizationVO(ouXml);
	...
	private void importUsers() throws Exception {
		URI uri = new URIBuilder("https://qa-coreservice.mpdl.mpg.de/aa/user-accounts")
				.addParameter("maximumRecords", String.valueOf(5000)).addParameter("startRecord", String.valueOf(1))
				.build();
		...
		SearchRetrieveResponseVO<AccountUserVO> userList = XmlTransformingService
				.transformToSearchRetrieveResponseAccountUser(xml);
		for (SearchRetrieveRecordVO<AccountUserVO> accountUser : userList.getRecords()) {
			...
			uri = new URIBuilder(
					"https://qa-coreservice.mpdl.mpg.de/aa/user-account/" + objectId + "/resources/current-grants")
			...
			uri = new URIBuilder(
					"https://qa-coreservice.mpdl.mpg.de/aa/user-account/" + objectId + "/resources/attributes")
							.addParameter("maximumRecords", String.valueOf(5000))
							.addParameter("startRecord", String.valueOf(1)).build();
	...
	private void saveAllVersionsOfPubItem(PubItemVO pubItemVo) {
		...
		for (int i = versionNumber; i > 0; i--) {
			try {
				...
				URI itemUri = new URIBuilder("https://qa-coreservice.mpdl.mpg.de/ir/item/" + objectId).build();
	...
	private static ItemVersionVO transformToNew(de.mpg.mpdl.inge.model.valueobjects.publication.PubItemVO itemVo) {
		...
		if (itemVo.getLatestRelease() != null) {
			if (itemVo.getLatestRelease().getVersionNumber() == itemVo.getVersion().getVersionNumber()) {
				...
				latestRelease
						.setObjectId(changeId(ID_PREFIX.ITEM.getPrefix(), itemVo.getLatestRelease().getObjectId()));
