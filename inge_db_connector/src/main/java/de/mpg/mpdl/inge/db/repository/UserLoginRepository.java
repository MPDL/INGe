...
public class UserLoginRepository {
  ...
  public void removeLogin(String loginName) {
    this.entityManager.createNativeQuery("DELETE FROM user_login WHERE loginname=?1").setParameter(1, loginName).executeUpdate();
  }

  @Transactional
  public void updateLogin(String loginName, String encodedPassword, LocalDate lastPasswordChange, boolean passwordChangeFlag)
      throws IngeTechnicalException {
    logger.info("Connection: " + this.entityManager.getDelegate());
    int rows = this.entityManager
  ...
  public String findPassword(String loginName) {
    ...
    List<?> result = this.entityManager.createNativeQuery("SELECT password FROM user_login WHERE loginname=?1").setParameter(1, loginName)
        .getResultList();
    if (!result.isEmpty()) {
  ...
  public LocalDate findLastPasswordChange(String loginName) {
    ...
    List<?> result = this.entityManager.createNativeQuery("SELECT last_password_change FROM user_login WHERE loginname=?1")
  ...
  public boolean findPasswordChangeFlag(String loginName) {
    ...
    List<?> result = this.entityManager.createNativeQuery("SELECT password_change_flag FROM user_login WHERE loginname=?1")
        .setParameter(1, loginName).getResultList();
    if (!result.isEmpty()) {
