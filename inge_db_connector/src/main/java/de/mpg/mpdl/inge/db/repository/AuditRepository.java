...
public interface AuditRepository extends JpaRepository<AuditDbVO, String> {
  ...
  @Query(
      value = "SELECT DISTINCT a.id, a.comment, a.event, a.modificationdate, a.modifier_name, a.modifier_objectid, a.pubitem_objectid, a.pubitem_versionnumber "
          + //
          "FROM audit_log a WHERE a.pubitem_objectid = ?1 ORDER BY a.modificationdate DESC",
      nativeQuery = true)
