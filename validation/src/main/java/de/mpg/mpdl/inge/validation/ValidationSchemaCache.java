...
import de.mpg.mpdl.inge.validation.xmltransforming.ConeContentHandler;
...
public final class ValidationSchemaCache {
  ...
  /**
   * Retrieve the precompiled schematron validation schema according to the given schemaName and
   * content-type.
   * 
   ...
   */
  public String getPrecompiledSchema(final String context, final String contentType,
      final String validationPoint) throws TechnicalException, ValidationSchemaNotFoundException {

    String sql =
        "SELECT escidoc_validation_schema_snippets.snippet_content "
    ...
    try {
      ...
      if (rs.next()) {
        ...
        connection.close();
        throw new ValidationSchemaNotFoundException("No schema for schemaName=" + context
            + ", content-type=" + contentType + " and validationPoint=" + validationPoint);
      }
   ...
   * @throws ValidationSchemaNotFoundException Schema was not found in the database.
   */
  public Transformer getPrecompiledTransformer(final String context, final String contentType,
      final String validationPoint) throws TechnicalException, ValidationSchemaNotFoundException {
   ...
   * @throws ValidationSchemaNotFoundException Schema was not found in the database.
   */
  public String getValidationSchema(final String schemaName, final String contentModel)
      throws TechnicalException, ValidationSchemaNotFoundException {

    String sql =
        "SELECT schema_content FROM escidoc_validation_schema"
   ...
   * @throws ValidationSchemaNotFoundException Schema was not found in the database.
   */
  public void setValidationSchema(final String schemaName, final String contentModel,
      final String content) throws TechnicalException, ValidationSchemaNotFoundException {
    ...
    try {
      ...
      if (rs.next()) {
        if (rs.next()) {
          ...
          // Update
          sql =
              "UPDATE escidoc_validation_schema SET schema_content = ?"
                  + " WHERE id_content_type_ref = ? and id_context_ref = ?";
        ...
        // Insert
        sql =
            "INSERT INTO escidoc_validation_schema "
  ...
  public synchronized void refreshCache(Date lastRefresh) throws Exception {
    ...
    if (lastRefresh == null) {
      ...
      try {
        String sql =
            "SELECT MAX(date_last_refreshed) AS date_last_refreshed FROM escidoc_validation_schema";
  ...
  private void retrieveNewSchemas(Date lastRefresh) throws Exception {
    ...
    if (newSchemas != null) {
      for (CacheTuple id : newSchemas.keySet()) {
        LOGGER.info("Creating/updating validation schema: cm=" + id.getContentModel() + "; name="
            + id.getSchemaName());
   ...
   */
  private void precompile(final String context, final String contentType,
   ...
   */
  private void transformSchema(String schema, final String context, final String contentType,
      String metadataVersion) throws TechnicalException, SQLException {
    String sql;
    ...
    try {
      ...
      sql =
          "DELETE FROM escidoc_validation_schema_snippets WHERE id_context_ref = ? AND id_content_type_ref = ?";
      pstmt = connection.prepareStatement(sql);
      ...
      for (int i = 0; i < phases.length; i++) {
        ...
        StringWriter precompiled =
            XsltTransforming.transform(schema, getSchematronTemplate(), params);

        sql =
            "INSERT INTO escidoc_validation_schema_snippets (id_context_ref, id_content_type_ref, "
  ...
  private Transformer getSchematronTemplate() throws TechnicalException {
    if (schematronTemplate == null) {
      try {
        InputStream fileIn =
            ResourceUtil.getResourceAsStream("stylesheet/validation_report.xsl",
                ValidationSchemaCache.class.getClassLoader());
  ...
  private Transformer getPhaseTemplate() throws TechnicalException {
    if (phaseTemplate == null) {
      ...
      try {
        InputStream fileIn =
            ResourceUtil.getResourceAsStream("stylesheet/validation_points.xsl",
                ValidationSchemaCache.class.getClassLoader());
  ...
  private void precompileAll() throws TechnicalException {
    ...
    String sql =
        "SELECT id_context_ref, id_content_type_ref, id_metadata_version_ref FROM escidoc_validation_schema";
    Connection connection = getConnection();
    try {
