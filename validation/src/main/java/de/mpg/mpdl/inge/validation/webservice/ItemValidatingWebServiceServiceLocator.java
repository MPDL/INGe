...
public class ItemValidatingWebServiceServiceLocator extends Service implements
  ...
  private String validation_address =
      "http://localhost:8080/validation_service/services/validation";
  ...
  public Remote getPort(Class serviceEndpointInterface) throws ServiceException {
    try {
      if (ItemValidatingWebService.class.isAssignableFrom(serviceEndpointInterface)) {
        ValidationSoapBindingStub _stub =
            new ValidationSoapBindingStub(new URL(validation_address), this);
        _stub.setPortName(getvalidationWSDDServiceName());
        return _stub;
      }
    } catch (Throwable t) {
